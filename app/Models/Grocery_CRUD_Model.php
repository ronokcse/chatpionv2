...
class Grocery_CRUD_Model extends Model
{
	...
	protected $db;
	protected $current_builder = null;
    ...
    {
    	// This method is typically called before get(), so we need to track the builder
    	// For now, we'll use a static/class property or pass builder through
    	// Since this is used in a chain, we'll need to handle it differently
    	// For CI4, we need to use the builder pattern properly
    	if (!isset($this->current_builder)) {
    		$this->current_builder = $this->db->table($this->table_name);
    	}
    	$this->current_builder->orderBy( $order_by , $direction );
    }
    ...
    function where($key, $value = NULL, $escape = TRUE)
    {
    	...
    	$this->current_builder->where( $key, $value, $escape);
    }
    ...
    function or_where($key, $value = NULL, $escape = TRUE)
    {
    	...
    	$this->current_builder->orWhere( $key, $value, $escape);
    }
    ...
    function having($key, $value = NULL, $escape = TRUE)
    {
    	...
    	$this->current_builder->having( $key, $value, $escape);
    }
    ...
    function or_having($key, $value = NULL, $escape = TRUE)
    {
    	...
    	$this->current_builder->orHaving( $key, $value, $escape);
    }
    ...
    function like($field, $match = '', $side = 'both')
    {
    	...
    	$this->current_builder->like($field, $match, $side);
    }
    ...
    function or_like($field, $match = '', $side = 'both')
    {
    	...
    	$this->current_builder->orLike($field, $match, $side);
    }
    ...
    function limit($value, $offset = '')
    {
    	...
    	$this->current_builder->limit( $value , $offset );
    }
    ...
    function get_edit_values($primary_key_value)
    {
    	...
    	$builder = $this->db->table($this->table_name);
    	$builder->where($primary_key_field,$primary_key_value);
    ...
    function join_relation($field_name , $related_table , $related_field_title)
    {
		...
		if($related_primary_key !== false)
		{
			...
			$this->current_builder->join( $related_table.' as '.$unique_name , "$unique_name.$related_primary_key = {$this->table_name}.$field_name",'left');
    ...
    function get_relation_array($field_name , $related_table , $related_field_title, $where_clause, $order_by, $limit = null, $search_like = null)
    {
    	...
    	if($search_like !== null)
    		$builder->having("$field_name_hash LIKE '%".$this->db->escapeLikeString($search_like)."%'");

    	if($order_by !== null)
    		$builder->orderBy($order_by);
    	else
    		$builder->orderBy($field_name_hash);
    ...
    function get_relation_n_n_selection_array($primary_key_value, $field_info)
    {
    	...
    	$builder = $this->db->table($field_info->relation_table);
    	$builder->select('*, '.$select,false);
    	...
    	{
    		if(!$use_template){
    			$builder->orderBy("{$field_info->selection_table}.{$field_info->title_field_selection_table}");
    		}
    	}
    	else
    	{
    		$builder->orderBy("{$field_info->relation_table}.{$field_info->priority_field_relation_table}");
    	}
    	$builder->where($field_info->primary_key_alias_to_this_table, $primary_key_value);
    	$builder->join(
    ...
    function get_relation_n_n_unselected_array($field_info, $selected_values)
    {
    	...
    	$builder = $this->db->table($field_info->selection_table);
    	$builder->select('*, '.$select,false);

    	if($use_where_clause){
    		$builder->where($field_info->where_clause);
    	}
    ...
    {
    	$builder = $this->db->table($field_info->relation_table);
    	$builder->where($field_info->primary_key_alias_to_this_table, $main_primary_key);
    	if(!empty($post_data))
    		$builder->whereNotIn($field_info->primary_key_alias_to_selection_table , $post_data);
    	$builder->delete();
    	...
    	if(!empty($post_data))
    	{
    		foreach($post_data as $primary_key_value)
	    	{
	    		...
	    		$check_builder = $this->db->table($field_info->relation_table);
	    		$check_builder->where($where_array);
				$count = $check_builder->countAllResults();
				...
				if($count == 0)
				{
					...
					$insert_builder = $this->db->table($field_info->relation_table);
					$insert_builder->insert($where_array);

				}elseif($count >= 1 && !empty($field_info->priority_field_relation_table))
				{
					$update_builder = $this->db->table($field_info->relation_table);
					$update_builder->where($where_array);
					$update_builder->update( array($field_info->priority_field_relation_table => $counter) );
				}
    ...
    function db_update($post_array, $primary_key_value)
    {
    	...
    	$builder = $this->db->table($this->table_name);
    	$builder->where($primary_key_field, $primary_key_value);
    	return $builder->update($post_array);
    }

    function db_insert($post_array)
    {
    	$builder = $this->db->table($this->table_name);
    	$insert = $builder->insert($post_array);
    	if($insert)
    	{
    ...
    function db_delete($primary_key_value)
    {
        ...
        if(!$this->db->fieldExists('deleted', $this->table_name)) return false;
       
        $builder = $this->db->table($this->table_name);
        $builder->where(array( $primary_key_field => $primary_key_value));
        $builder->update(array("deleted" => "1") );
    ...
    {
    	$builder = $this->db->table($this->table_name);
    	$builder->where($field_name, $filename);
    	if( $builder->update(array($field_name => '')) )
    	{
    		return true;
    	}
    	else
