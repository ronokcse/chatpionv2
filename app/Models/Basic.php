...
class Basic extends Model
{
	...
	public function generate_where_clause($where, $builder = null) //generates the joining clauses as given array
	{
		...
		for($i=0;$i<count($keys);$i++)
		{
			...
			else if($keys[$i]=='where_not_in') // works similar as where_in specified above
			{
				...
				$builder->orWhere($where['or_where']); // genereates the or_where clauses
			...
			else if($keys[$i]=='or_where_advance') // works similar as where_in but the array indexes & values are in reverse format as given parameter 
			{
				...
				for($j=0;$j<count($keys_inner);$j++)
				{
					...
					$value=$keys_inner[$j];					
					$builder->orWhere($field,$value);	// genereates the or_where clauses								
				} // end for
			...
			else if($keys[$i]=='or_where_in') // works similar as where_in specified above
			{
				...
				for($j=0;$j<count($keys_inner);$j++)
				{
					...
					$builder->orWhereIn($field,$value);	// genereates the or_where_in clauses					
				} // end for
			...
			else if($keys[$i]=='or_where_not_in') // works similar as where_in specified above
			{
				...
				for($j=0;$j<count($keys_inner);$j++)
				{
					...
					$builder->orWhereNotIn($field,$value);	// genereates the or_where_not_in clauses					
				} // end for
	...
	public function generate_joining_clause($join, $builder = null) //generates the joining clauses as given array
	{
	...
	public function get_data($table,$where='',$select='',$join='',$limit='',$start=NULL,$order_by='',$group_by='',$num_rows=0,$csv='') //selects data from a table as well as counts number of affected rows
	{
		...
		$result_array=$query->getResultArray(); //fetches the rows from database and forms an array[]
		
		if($num_rows==1)
		{
			$num_rows=$query->getNumRows(); //counts the affected number of rows
