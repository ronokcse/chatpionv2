...

    {
      ...
      {          
...

        {
          ...
          if(!empty($coupon_data) && $coupon_amount>0)
          {
            if($coupon_product_ids=="0")
            {
              $coupon_applies = true; // Applies to all products
            }
            else
            {
              // Check if any product in cart matches coupon product restrictions
              foreach($cart_item_data as $key => $value)
              {
                if(in_array($value['product_id'], $coupon_product_ids_array))
                {
                  $coupon_applies = true;
                  break;
                }
...

          {
...

            {
              ...
              $coupon_info = "";
              // Use sell_price if available and less than original_price, otherwise use original_price
              $base_display_price = ($sell_price > 0 && $sell_price < $original_price) ? $sell_price : $original_price;
              $display_price = $base_display_price; // Price to show in cart (may be discounted for display)
              // Use base_display_price (sell_price if available, otherwise original_price) for subtotal calculation
              // Coupon discount will be applied only once to full checkout amount
              $calculation_price = $base_display_price; // Price to use for subtotal calculation (base price without coupon discount)

              // Calculate discount for display purposes on products
              // But actual discount calculation will be on full checkout amount
              if($coupon_applies && ($coupon_product_ids=="0" || in_array($product_id, $coupon_product_ids_array)))
              {
                if($coupon_type=="percent")
                {
                  $disc = ($display_price*$coupon_amount)/100;
                  if($disc<0) $disc=0;
                  $display_price = $display_price - $disc;
                  if($display_price<0) $display_price = 0;
                ...
                else if($coupon_type=="fixed cart")
                {
                  // For fixed cart, show coupon code but discount will be on full amount
                  $coupon_info = $coupon_code_new;
                }
              }

              // Use display_price for showing in cart, but calculation_price (original) for subtotal
              $new_price = mec_number_format($display_price,$decimal_point,'0');
            ...
            // Use calculation_price (base price without coupon discount) for subtotal
            // Coupon discount will be applied only once to full checkout amount (subtotal + shipping + tax)
            $total_price = $calculation_price*$value["quantity"];

            $subtotal+=$total_price;



            // For tax calculation, use base display price (sell_price if available, otherwise original_price)
            if($product_list_assoc[$product_id]["taxable"]=='1') $taxable_amount+=$base_display_price*$value["quantity"];
        ...
        // Apply coupon discount to full checkout amount (subtotal + shipping + tax)
        if($coupon_applies)

        {
...

          {
            ...
            $discount = ($payment_amount*$coupon_amount)/100;
