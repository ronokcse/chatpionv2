...
class Dashboard extends Home
{
    ...
    */
    ...
    public function index($default_value='0')
    {
        ...
        if(session()->get('user_type') != 'Admin') $default_value='0';
        if($default_value == '0')
        {
        ...
        // Start Third block section (ecommerce statistics section)
            $store_currency_query = $this->db->table('ecommerce_config')->select('currency')->distinct()->where('user_id', $user_id)->orderBy('currency', 'desc')->get();
            $store_currency = array_column($store_currency_query->getResultArray(), 'currency');
            ...
            if(session()->get('ecommerce_dashboard_currency')) 
                $selected_curr = session()->get('ecommerce_dashboard_currency');
        ...
        // Start Fourth section (ecommerce total earning graphical 30 days)
            // CI4 Query Builder format
            $cart_data_graph = $this->db->table('ecommerce_cart')
                ->select('ecommerce_cart.*, messenger_bot_subscriber.id as subscriber_table_id, messenger_bot_subscriber.first_name, messenger_bot_subscriber.last_name, messenger_bot_subscriber.full_name, messenger_bot_subscriber.profile_pic, messenger_bot_subscriber.email, messenger_bot_subscriber.image_path, messenger_bot_subscriber.subscribe_id, messenger_bot_subscriber.social_media')
                ->join('messenger_bot_subscriber', 'messenger_bot_subscriber.subscribe_id = ecommerce_cart.subscriber_id', 'left')
                ->where('ecommerce_cart.user_id', $user_id)
                ->where('ecommerce_cart.updated_at >=', $days_30_before)
                ->where('ecommerce_cart.updated_at <=', $today)
                ->where('ecommerce_cart.currency', $selected_curr)
                ->get()
                ->getResultArray();
            ...
            // CI4 Query Builder format
            $data['top_products'] = $this->db->table('ecommerce_cart_item')
                ->select('ecommerce_product.product_name, ecommerce_product.thumbnail, ecommerce_product.woocommerce_product_id, sum(quantity) as sales_count, ecommerce_cart_item.*')
                ->join('ecommerce_product', 'ecommerce_cart_item.product_id = ecommerce_product.id', 'left')
                ->where('ecommerce_product.user_id', $user_id)
                ->where('ecommerce_cart_item.updated_at >=', $days_30_before)
                ->where('ecommerce_cart_item.updated_at <=', $today)
                ->groupBy('ecommerce_cart_item.product_id')
        ...
        // Fifth Item section (Subscribers combined chart section 12 months)
            ...
            // CI4 Query Builder format
            $subscribers = $this->db->table('messenger_bot_subscriber')
                ->select('COUNT(id) as subscriber_gain, SUM(email != "") as email_gain, SUM(phone_number != "") as phone_gain, month(subscribed_at) as month_number')
                ->where('user_id', $user_id)
                ->where('subscribed_at >= DATE_SUB(NOW(),INTERVAL 11 MONTH)', null, false)
                ->where('is_bot_subscriber', '1')
                ->groupBy('month(subscribed_at)')
                ->orderBy('month(subscribed_at)', 'ASC')
                ->get()
                ->getResultArray();
        ...
        // sixth item section [latest subscribers]
            ...
            foreach($latest_subscriber_info as $value)
            {
                ...
                $page_table_id = $value['page_table_id'] ?? null;
                $latest_subscriber_list[$i]['page_name'] = isset($page_list[$page_table_id]) ? $page_list[$page_table_id]['page_name'] : '';
                $latest_subscriber_list[$i]['page_id'] = isset($page_list[$page_table_id]) ? $page_list[$page_table_id]['page_id'] : '';
        ...
        // seventh + eight item section [Different sources of subscribers 12 months]
            ...
            $refferer_source_info['checkbox_plugin']['title'] = lang("Checkbox Plugin");
            $refferer_source_info['customer_chat_plugin']['title'] = lang("Customer Chat Plugin");
            $refferer_source_info['sent_to_messenger']['title'] = lang("Sent to Messenger Plugin");
            $refferer_source_info['me_link']['title'] = lang("m.me Link");
            $refferer_source_info['direct']['title'] = lang("Direct From Facebook");
            $refferer_source_info['comment_private_reply']['title'] = lang("Comment Private Reply");
            ...
            // CI4: Add conditions to where array instead of using $this->db->where()
            $where['where']['subscribed_at >= DATE_SUB(NOW(),INTERVAL 11 MONTH)'] = null;
            $where['where']['is_bot_subscriber'] = '1';
        ...
        // Eighth section start
            $subscribers_source_info = array(
                lang('Checkbox') => $refferer_source_info['checkbox_plugin']['subscribers'],        
                lang('Customer Chat') => $refferer_source_info['customer_chat_plugin']['subscribers'],     
                lang('Sent to Messenger') => $refferer_source_info['sent_to_messenger']['subscribers'],        
                lang('m.me') => $refferer_source_info['me_link']['subscribers'],        
                lang('Direct') => $refferer_source_info['direct']['subscribers'],        
                lang('Comment Reply') => $refferer_source_info['comment_private_reply']['subscribers'],      
            );
       ...
       if(check_module_access($module_id=82,true)) {
           ...
           $unread_count_query = $this->db->table('messenger_bot_subscriber')
               ->select('SUM(unseen_count) as total_unseen')
               ->where('user_id', $user_id);
           if (!empty($allowed_bot_ids)) {
               $unread_count_query->whereIn('page_table_id', $allowed_bot_ids);
           }
           $unread_count_query->groupBy('user_id');
           $unread_count_data = $unread_count_query->get()->getRow();

           $whatsapp_unread_query = $this->db->table('messenger_bot_subscriber')
               ->select('*')
               ->where('user_id', $user_id)
               ->where('unseen_count >', 0);
           if (!empty($allowed_bot_ids)) {
               $whatsapp_unread_query->whereIn('page_table_id', $allowed_bot_ids);
           }
           $whatsapp_unread_query->orderBy('last_subscriber_interaction_time', 'DESC')
               ->limit(50);
           $whatsapp_unread_data = $whatsapp_unread_query->get()->getResult();
        ...
        $data['user_id'] = $this->user_id; // Pass user_id to view
        $data['user_id_url'] = $this->uri->segment(3) ?? 0; // Pass user_id_url to view
    ...
    public function get_first_div_content()
    {
        ...
        if(!empty($user_id_url) && $user_id_url>0 && session()->get('user_type')=='Admin') $user_id = $user_id_url;
    ...
    */
    ...
    public function set_currency()
    {
        ...
        session()->set('ecommerce_dashboard_currency',$currency);
        echo '1';
    }
