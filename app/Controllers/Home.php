...
class Home extends BaseController
{
    ...
    public function initController(RequestInterface $request, ResponseInterface $response, LoggerInterface $logger)
    {
        ...
        if ($seg != "installation" && $seg != "installation_action" && $seg != "central_webhook_callback" && $seg != "webhook_callback_main") {
            if (file_exists(ROOTPATH . 'app/install.txt')) {
                return redirect()->to('home/installation');
            }
        }
        ...
        if ($seg != "installation" && $seg != "installation_action") {
            ...
            $this->real_user_id = $this->session->get("real_user_id");
            $this->is_manager = $this->session->get("is_manager") ?? 0;
            $this->upload_path = realpath(ROOTPATH . 'upload');
            $this->session->remove('set_custom_link');
            ...
            */
            ...
            if ($this->session->get('logged_in') == 1) {
                $package_info = $this->is_manager == 1 ? $this->session->get("role_info") : $this->session->get("package_info");
                $module_ids = '';
                if ($this->is_manager == 1) {
                    ...
                    $this->team_allowed_pages = $this->session->get('team_allowed_pages') ?? [];
                    $this->session->set('module_access', $this->module_access);
                    $this->session->set('team_access', $this->team_access);
                } else {
                    if (isset($package_info["module_ids"])) $module_ids = $package_info["module_ids"];
                    $this->module_access = explode(',', $module_ids);
                    $this->session->set('module_access', $this->module_access);
                    $this->session->set('team_access', []);
                }
            }
        }
        ...
        if (($this->config->force_https ?? '0') == '1') {
            $actualLink = base_url(uri_string());
        ...
        if ($this->session->get('log_me_out') == '1') $this->logout();

        if ($this->session->get('csrf_token_session') == "") $this->session->set('csrf_token_session',  bin2hex(random_bytes(32)));
        ...
        //     $strict_ajax_call = ($this->config->strict_ajax_call ?? null);
        ...
        $this->session->set("is_mobile", $is_mobile);
    }
    ...
    public function assign_conversation_to_team_member($subscriber_id, $team_member_id, $page_table_id, $team_assign_role_id, $social_media_type)
    {
        if (isset($team_assign_role_id) && $team_assign_role_id > 0) {
            $team_member_list = $this->db->table('team_members')
                ->select('id, name')
                ->where('team_role_id', $team_assign_role_id)
                ->get()
                ->getResult();
        ...
        if (!isset($team_member_name) || empty($team_member_name)) {
            ...
            $query = $this->db->table('team_members')
                ->select('id, name')
                ->where('id', $team_member_id)
                ->get();
            $team_member_list = $query->getRow();
        ...
        if ($team_member_id > 0) {
            ...
            $this->db->table('messenger_bot_subscriber')
                ->where(['subscribe_id' => $subscriber_id, 'page_table_id' => $page_table_id])
                ->update(["assigned_used_id" => $team_member_id]);
    ...
    public function agent_assign_notifications($social_media = 'fb', $assigned_used_id = null, $subscriber_id = null)
    {
        ...
        $title =  $social_media_name . " " . lang('Livechat Assigned');
        $color_class = $social_media == 'ig' ? 'warning' : 'primary';
        $icon_class = $social_media == 'ig' ? 'fab fa-instagram' : 'fab fa-facebook';
        $message_content = lang('You have been assigned to ') . " <b>" . $subscriber_name . "</b> " . $social_media_name . " Livechat Agent.";
    ...
    public function _language_loader($default_lang = "")
    {
        ...
        $config_lang = $this->config->language ?? null;
        if (!$config_lang || $config_lang == "")
            $this->language = "english";
        else $this->language = $config_lang;

        if ($this->session->get("selected_language") != "")
            $this->language = $this->session->get("selected_language");
        else if (!$config_lang || $config_lang == "")
            $this->language = "english";
        else $this->language = $config_lang;
        ...
        $path = str_replace('\\', '/', ROOTPATH . 'app/language/' . $this->language);
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                if (!$con) {
                    ...
                    $this->session->set('mysql_error', $mysql_error);
                    return $this->installation();
                }
                if (!@mysqli_select_db($con, $database_name)) {
                    $this->session->set('mysql_error', "Database not found.");
                    return $this->installation();
                }
                mysqli_close($con);
                ...
                file_put_contents(ROOTPATH . 'app/config/database.php', $database_data, LOCK_EX);
                //writting application/config/database
                ...
                if (file_exists(ROOTPATH . 'app/install.txt')) {
                    unlink(ROOTPATH . 'app/install.txt');
                }
                //deleting the install.txt file,because installation is complete
                return redirect()->to('home/login');
            }
    ...
    public function index()
    {
        ...
        else return $this->_site_viewcontroller();
    }


    public function _time_zone_set()
    {
        $time_zone = ($this->config->time_zone ?? null);
        if ($time_zone == '') {
    ...
    {
        return view('page/error', array("page_title" => lang("Access Denied"), "message" => lang("You do not have permission to access this content")));
    }
    ...
    {
        $current_theme = ($this->config->current_theme ?? null);
        ...
        $data['basic'] = $this->basic ?? null;
        $data['module_access'] = $this->module_access ?? [];
        $data['user_id'] = $this->user_id ?? 0;
        $data['is_manager'] = $this->is_manager ?? 0;
        echo view($theme_load, $data);
    }

    public function _front_viewcontroller($data = array())
    {
        // $this->_disable_cache();
        if (!isset($data['body']))   $data['body'] = ($this->config->default_page_url ?? null);
        if (!isset($data['page_title'])) $data['page_title'] = "";
        ...
        if (file_exists(ROOTPATH . 'app/core/licence_type.txt'))
    ...
    public function _viewcontroller($data = array())
    {
        ...
        if (!isset($data['page_title'])) {
            $data['page_title'] = lang("Admin Panel");
        }


        if ($this->session->get('download_id_front') == "")
            $this->session->set('download_id_front', md5(time() . $this->_random_number_generator(10)));

        if ($this->session->get('user_type') == 'Admin' || in_array(65, $this->module_access)) {
        ...
        // $where_custom is handled in get_data method
        $where_announcement = ['where' => $where_custom];
        $data['annoucement_data'] = $this->basic->get_data("announcement", $where_announcement, $select = '', $join = '', $limit = '', $start = NULL, $order_by = 'created_at DESC');
        ...
        // Pass controller properties to view for js_variables.php and theme files
        $data['user_id'] = $this->user_id ?? 0;
        $data['language'] = $this->language ?? 'english';
        $data['is_demo'] = $this->is_demo ?? '0';
        $data['uri'] = $this->uri; // Pass URI compatibility object
    ...
    public function _site_viewcontroller($data = array())
    {
        ...
        $this->session->set("contact_captcha", $contact_captcha);
        $data["language_info"] = $this->_language_list();
    ...
    public function login_page($is_team_login = '0')
    {
        ...
        if ($this->session->get('logged_in') == 1) return redirect()->to('dashboard');
        ...
        if (!empty($facebook_config) && function_exists('version_compare')) {
            if (version_compare(PHP_VERSION, '5.4.0', '>=')) {
                $this->session->set('social_login_session_set', 1);
        ...
        return $this->_subscription_viewcontroller($data);
    }
    ...
    public function login($is_team_login = '0') //loads home view page after login (this )
    {
        ...
        if ($this->session->get('logged_in') == 1 && $this->session->get('user_type') == 'Admin') {
            return redirect()->to('dashboard');
        }
        if ($this->session->get('logged_in') == 1 && $this->session->get('user_type') == 'Member') {
            return redirect()->to('dashboard');
        }

        // Only run validation if it's a POST request
        if ($this->request->getMethod() === 'POST') {
            $this->form_validation->set_rules('username', '<b>' . lang("email") . '</b>', 'trim|required');
            $this->form_validation->set_rules('password', '<b>' . lang("password") . '</b>', 'trim|required');
        ...
        // POST request with valid data - process login
        {
            ...
            if ($is_team_login == '1') {
                ...
                if (($this->config->master_password ?? null) != '') {
                    if (md5($_POST['password']) == ($this->config->master_password ?? null))
            ...
            if ($count == 0) {
                $this->session->set('login_msg', lang("invalid email or password"));
                redirect(uri_string());
            } else {
                ...
                if ($is_team_login == '1') {
                    ...
                    $this->session->set('real_user_type', $user_type);
                } else {
                ...
                $this->session->set('team_username', $team_member_name);
                $this->session->set('user_id', $user_id);
                $this->session->set('download_id', time());
                $this->session->set('user_login_email', $info[0]['email']);
                $this->session->set('expiry_date', $info[0]['expired_date']);
                $this->session->set('brand_logo', $logo);
                $this->session->set('selected_global_media_type', 'fb');
                if ($is_team_login == '1') {
                    $this->session->set('is_manager', 1);
                    $this->session->set('real_user_id', $real_user_id);
                } else {
                    $this->session->set('is_manager', 0);
                    $this->session->set('real_user_id', $user_id);
                }
                ...
                if (($this->config->backup_mode ?? null) == 0)  // users will use admins app
                {
                    ...
                    $this->session->set("fb_rx_login_database_id", $facebook_rx_config_id);

                    if (isset($fb_info[0])) $facebook_rx_fb_user_info = $fb_info[0]["id"];
                    else $facebook_rx_fb_user_info = 0;
                    $this->session->set("facebook_rx_fb_user_info", $facebook_rx_fb_user_info);  // this is used in account fb switchig
                } else  // users will use own app
                {
                    $fb_info_admin = $this->basic->get_data("facebook_rx_config", array("where" => array("status" => '1', 'user_id' => $this->session->get("user_id"), 'developer_access' => '0')), $select = '');
                ...
                $this->session->set('package_info', $package_info_session);
                if (isset($package_info_session["id"])) $this->session->set('current_package_id', $package_info_session["id"]);
                ...
                if ($is_team_login == '1') {
                    ...
                    $this->session->set('role_info', $role_info_session);
                    if (isset($role_info_session["id"])) $this->session->set('current_role_id', $role_info_session["id"]);
                    if (isset($role_info_session["page_ids"])) {
                        ...
                        $this->session->set('team_allowed_pages', $team_allowed_pages);
                    }
                }
    ...
    function google_login_back()
    {
        ...
        if (!empty($info) && isset($info->email) && isset($info->name)) {
            ...
            if (!$this->basic->is_exist("users", array("email" => $info->email))) {
                ...
                $userid_through_affiliate = $this->db->insertID();
                ...
                $mail_service_id = ($this->config->mail_service_id ?? null);
                $system_short_name = ($this->config->product_short_name ?? null);
            ...
            if ($count == 0) {
                ...
                $this->session->set('logged_in', 1);
                $this->session->set('username', $username);
                $this->session->set('team_username', "");
                $this->session->set('user_type', $user_type);
                $this->session->set('user_id', $user_id);
                $this->session->set('download_id', time());
                $this->session->set('user_login_email', $info[0]['email']);
                ...
                {
                    $fb_info_admin = $this->basic->get_data("facebook_rx_config", array("where" => array("status" => '1', 'user_id' => $this->session->get("user_id"))), $select = '');
    ...
    public function facebook_login_back()
    {
        $this->session->set('social_login_session_set', 1);
        $this->load->library('Fb_rx_login');
        $config_id = $this->session->get('return_configid_used_for_social_login');
        ...
        if (is_array($info) && !empty($info) && isset($info["name"])) {
            ...
            // SQL where clause handled in is_exist method
            if ($this->basic->is_exist("users", $sql)) {
                ...
                if (isset($access_token)) {
                    ...
                    if (empty($exist_or_not)) {
                        ...
                        if ($status == "2") {
                            $this->session->set('limit_cross', lang("Module limit is over."));
                    ...
                    $this->session->set("facebook_rx_fb_user_info", $facebook_table_id);
                    ...
                    if (!empty($page_list)) {
                        foreach ($page_list as $page) {
                            ...
                            if (($this->config->instagram_reply_enable_disable ?? null) == '1')
            ...
            if ($count == 0) {
                ...
                if ($user_type == 'Admin') {
                    $this->session->set('login_msg', lang("You have admin account in this system, please login to your admin account."));
                    return redirect()->to("home/login_page");
                }
                ...
                $this->session->set('user_login_email', $info[0]['email']);
                $this->session->set('expiry_date', $info[0]['expired_date']);
                $this->session->set("fb_rx_login_database_id", $config_id);
                $this->session->set('selected_global_media_type', 'fb');
                $this->session->set('is_manager', 0);
                $this->session->set('real_user_id', $user_id);
            ...
            $this->session->set('login_msg', lang("Could not fetch data from your Facebook account. Please try again later by giving necessery permissions."));
            return redirect()->to("home/login_page");
        }
    }
    ...
    public function get_broadcast_tags($media_type = 'fb')
    {
        ...
        unset($new_tags[""]);
        $dropdown = array("" => lang("Select Tag"), "New Tags" => $new_tags, "Tags supported until Mar 4,2020" => $old_tags);
        return $dropdown;
    }
    ...
    protected function time_zone_drop_down($datavalue = '', $primary_key = null, $mandatory = 0) // return HTML select
    {
        ...
        foreach ($all_time_zone as $zone_name => $value) {
            if ($primary_key != null) {
                ...
                if ($zone_name == ($this->config->time_zone ?? null)) {
                    $selected = " selected = 'selected' ";
                } else {
                    $selected = "";
                }
    ...
    function get_facebook_instagram_dropdown($facebook_rx_fb_user_info_id = 0, $dropdown_name = "page_id", $dropdown_id = "page_id", $dropdown_style = "", $dropdown_class = 'select2 form-control', $return_groups = false, $social_posting = 0)
    {
        ...
        if (($this->config->facebook_poster_group_enable_disable ?? null) == '' || ($this->config->facebook_poster_group_enable_disable ?? null) == '0') $has_group_access = false;
        ...
        if (!addon_exist($module_id = 320, $addon_unique_name = "instagram_bot") && !$return_groups)
            $str .= '<option value="">' . lang("Facebook Page") . '</option>';

        if (addon_exist($module_id = 320, $addon_unique_name = "instagram_bot") || $return_groups) {
            $str .= '<optgroup label="' . lang("Facebook Page") . '">';
            $str .= '<option value="all_pages">' . lang("ALL Pages") . '</option>';
        }

        foreach ($page_info as $key => $value) {
            $selected = (
                $this->session->get('selected_global_page_table_id') == $value['id'] &&
                ($this->using_media_type == 'fb' || $this->session->get('selected_global_media_type') == '')
            ) ? 'selected' : '';
        ...
        if (addon_exist($module_id = 320, $addon_unique_name = "instagram_bot")) {
            $str .= '<optgroup label="' . lang("Instagram") . '">';
            $str .= '<option value="all_accounts-ig">' . lang("ALL Accounts") . '</option>';

            foreach ($page_info as $key => $value) {
                $selected = ($this->session->get('selected_global_page_table_id') == $value['id'] && $this->using_media_type == 'ig') ? 'selected' : '';
        ...
        if ($return_groups) {
            ...
            $group_info = $this->basic->get_data("facebook_rx_fb_group_info", array("where" => array("user_id" => $this->user_id, "facebook_rx_fb_user_info_id" => $this->session->get("facebook_rx_fb_user_info"))), 'id,group_name');

            $str .= '<optgroup label="' . lang("Facebook Group") . '">';
            $str .= '<option value="all_groups-gr">' . lang("ALL Groups") . '</option>';
    ...
    {
        if ($this->session->get('logged_in') != 1) exit();
        $this->ajax_check();
        ...
        foreach ($template_types as $template) {
            $template_type_str .= '<option value="' . $template . '">' . lang($template) . '</option>';
        }
        ...
        if ($load_label == '0') {
            if (count($label_ids) > 0) {
                ...
                $included_label_ids_array = array_column($query1->getResultArray(), 'subscriber_table_id');

                // where_in will be handled in get_data method via where parameter
                // Store for later use in get_data
            }
            ...
            if (count($excluded_label_ids) > 0) {
                ...
                $excluded_label_ids_array = array_column($query2->getResultArray(), 'subscriber_table_id');

                // where_not_in will be handled in get_data method via where parameter
                // Store for later use in get_data
            }
        }
    ...
    public function code_genaration()
    {
        ...
        if ($result[0]['num'] == 1) {
            ...
            $productname = ($this->config->product_name ?? null);
            ...
            if ($this->basic->insert_data($table, $info)) {
                ...
                if (isset($email_template_info[0]) && $email_template_info[0]['subject'] != '' && $email_template_info[0]['message'] != '') {
                    ...
                    $message = "<p>" . lang('to reset your password please perform the following steps') . " : </p>
                                <ol>
                                    <li>" . lang("go to this url") . " : " . $url_final . "</li>
                                    <li>" . lang("enter this code") . " : " . $code . "</li>
                                    <li>" . lang("reset your password") . "</li>
                                </ol>
                                <h4>" . lang("link and code will be expired after 24 hours") . "</h4>";
                }


                $from = ($this->config->institute_email ?? null);
                $to = $email;
                $mask = ($this->config->product_name ?? null);
                $html = 1;
    ...
    function _mail_sender($from = '', $to = '', $subject = '', $message = '', $mask = "", $html = 1, $smtp = 1, $attachement = "", $test_mail = "")
    {
        if ($to != '' && $subject != '' && $message != '') {
            if (($this->config->email_sending_option ?? null) == '') $email_sending_option = 'smtp';
            else $email_sending_option = ($this->config->email_sending_option ?? null);

            if ($test_mail == 1) $email_sending_option = 'smtp';

            //  if($smtp != '1') $message=$message."<br/><br/>".lang("The email was sent by"). ": ".$from;
    ...
    {
        $signup_form = ($this->config->enable_signup_form ?? null);
        ...
        $this->session->set("sign_up_captcha", $captcha);

        $data["page_title"] = lang("Sign Up");
    ...
    public function affiliate_commission($affiliate_id = 0, $userid = 0, $event = '', $package_price = 0)
    {
        ...
        if ($this->db->trans_status() === FALSE) {
            echo json_encode(array('status' => '0', 'message' => lang('Database error. Something went wrong.')));
            exit();
        }
    }
    ...
    {
        $enable_signup_activation = ($this->config->enable_signup_activation ?? null);
        ...
        if ($_POST) {
            $this->form_validation->set_rules('name', '<b>' . lang("name") . '</b>', 'trim|required');
            $this->form_validation->set_rules('email', '<b>' . lang("email") . '</b>', 'trim|required|valid_email|is_unique[users.email]');
            // $this->form_validation->set_rules('mobile', '<b>'.lang("mobile").'</b>', 'trim');
            $this->form_validation->set_rules('password', '<b>' . lang("password") . '</b>', 'trim|required');
            $this->form_validation->set_rules('confirm_password', '<b>' . lang("confirm password") . '</b>', 'trim|required|matches[password]');
            $this->form_validation->set_rules('captcha', '<b>' . lang("captcha") . '</b>', 'trim|required|integer');
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if ($captcha != $this->session->get("sign_up_captcha")) {
                    $this->session->set("sign_up_captcha_error", lang("invalid captcha"));
                    return $this->sign_up();
                }
                ...
                if ($this->basic->insert_data('users', $data)) {
                    ...
                    if ($enable_signup_activation == '1') {
                        ...
                        if (isset($email_template_info[0]) && $email_template_info[0]['subject'] != '' && $email_template_info[0]['message'] != '') {
                            ...
                            $message = "<p>" . lang("to activate your account please perform the following steps") . "</p>
                                        <ol>
                                            <li>" . lang("go to this url") . ":" . $url_final . "</li>
                                            <li>" . lang("enter this code") . ":" . $code . "</li>
                                            <li>" . lang("activate your account") . "</li>
                                        </ol>";
                        }
                        ...
                        $this->session->set('reg_success', 1);
                        return $this->sign_up();
    ...
    public function email_contact()
    {
        ...
        if ($_POST) {
            ...
            $this->form_validation->set_rules('email',                    '<b>' . lang("email") . '</b>',              'trim|required|valid_email');
            $this->form_validation->set_rules('subject',                  '<b>' . lang("message subject") . '</b>',            'trim|required');
            $this->form_validation->set_rules('message',                  '<b>' . lang("message") . '</b>',            'trim|required');
            $this->form_validation->set_rules('captcha',                  '<b>' . lang("captcha") . '</b>',            'trim|required|integer');
            ...
            if ($this->form_validation->run() == false) {
                ...
                if ($captcha != $this->session->get("contact_captcha")) {
                    $this->session->set("contact_captcha_error", lang("invalid captcha"));
                ...
                $email = $this->input->post('email', true);
                $subject = ($this->config->product_name ?? null) . " | " . $this->input->post('subject', true);
                $message = $this->input->post('message', true);
                $message = $message . "<br/><br/>" . lang("The email was sent by") . ": " . $email;

                $this->_mail_sender($from = $email, $to = ($this->config->institute_email ?? null), $subject, $message, ($this->config->product_name ?? null), $html = 1);
    ...
    {
        $this->session->set('allow_cookie', 'yes');
    ...
    public function _insert_usage_log($module_id = 0, $usage_count = 0, $user_id = 0)
    {
        ...
        if ($user_id == 0) $user_id = $this->session->get("user_id");
    ...
    public function _check_usage($module_id = 0, $request = 0, $user_id = 0)
    {
        ...
        if ($this->session->get("package_info") != "") {
            $package_info = $this->session->get("package_info");
            if ($this->session->get('user_type') == 'Admin') return "1";
        } else {
    ...
    public function print_limit_message($module_id = 0, $request = 0)
    {
        ...
        if ($status == "2") {
            echo lang("sorry, your bulk limit is exceeded for this module.") . "<a href='" . site_url('usage_history') . "'>" . lang("click here to see usage log") . "</a>";
            exit();
        } else if ($status == "3") {
            echo lang("sorry, your monthly limit is exceeded for this module.") . "<a href='" . site_url('usage_history') . "'>" . lang("click here to see usage log") . "</a>";
            exit();
        }
    }
    ...
    {
        if ($this->session->get('logged_in') == 1 && $this->session->get('user_type') != 'Admin' && $this->is_manager != 1) {
            $where['where'] = array('id' => $this->session->get('user_id'));
            ...
            $package_data = $this->basic->get_data("users", $where = array("where" => array("users.id" => $this->session->get("user_id"))), $select = "package.price as price", $join = array('package' => "users.package_id=package.id,left"));
            ...
            if ($expire_date < $current_date && ($price > 0 && $price != ""))
                return redirect()->to('payment/buy_package');
        }
    }

    public function important_feature()
    {
        if (file_exists(ROOTPATH . 'app/config/licence.txt') && file_exists(ROOTPATH . 'app/core/licence.txt')) {
            $config_existing_content = file_get_contents(ROOTPATH . 'app/config/licence.txt');
            $config_decoded_content = json_decode($config_existing_content, true);

            $core_existing_content = file_get_contents(ROOTPATH . 'app/core/licence.txt');
            ...
            if ($config_decoded_content['is_active'] != md5($config_decoded_content['purchase_code']) || $core_decoded_content['is_active'] != md5(md5($core_decoded_content['purchase_code']))) {
                return redirect()->to("home/credential_check");
            }
        } else {
            return redirect()->to("home/credential_check");
        }
    ...
    {
        if ($this->is_demo == '1') return redirect()->to('home/access_forbidden');

        $permissio = 0;
        if ($this->session->get("user_type") == "Admin") $permissio = 1;
        else $permissio = 0;

        if ($permissio == 0) return redirect()->to('home/access_forbidden');

        $data["page_title"] = lang("Credential Check");
    ...
    public function code_activation_check_action($purchase_code, $only_domain, $periodic = 0)
    {
        ...
        if (!isset($decoded_credentials['error'])) {
            ...
            if ($content['status'] == 'success') {
                ...
                file_put_contents(ROOTPATH . 'app/config/licence.txt', $config_json_content_to_write, LOCK_EX);

                $content_to_write['is_active'] = md5(md5($purchase_code));
                $core_json_content_to_write = json_encode($content_to_write);
                file_put_contents(ROOTPATH . 'app/core/licence.txt', $core_json_content_to_write, LOCK_EX);
                ...
                file_put_contents(ROOTPATH . 'app/core/licence_type.txt', $encoded, LOCK_EX);

                return json_encode("success");
            } else if ($content['status'] == 'error') {
                if (file_exists(ROOTPATH . 'app/core/licence.txt')) unlink(ROOTPATH . 'app/core/licence.txt');
                return json_encode($content);
            }
        } else {
            if ($periodic == 1)
    ...
    {
        $file_data = file_get_contents(ROOTPATH . 'app/core/licence.txt');
    ...
    public function php_info()
    {
        if ($this->session->get('user_type') == 'Admin')
            echo phpinfo();
        else return redirect()->to('home/access_forbidden');
    }
    //=======================USAGE LOG & LICENSE FUNCTIONS======================
    //==========================================================================
    ...
    protected function addon_credential_check($purchase_code = "", $item_name = "")
    {
        ...
        if ($purchase_code == "") {
            echo json_encode(array('status' => '0', 'message' => lang('Add-on purchase code has not been provided.')));
            exit();
        }

        $item_name = urlencode($item_name);
        $only_domain = get_domain_only(site_url());
        $url = "https://xeroneit.net/development/envato_license_activation/purchase_code_check.php?purchase_code={$purchase_code}&domain={$only_domain}&item_name=XeroChat-{$item_name}";
        ...
        if (!isset($decoded_credentials['error'])) {
            $content = json_decode($decoded_credentials['content'], true);
            if ($content['status'] != 'success') {
                echo json_encode(array('status' => '0', 'message' => lang('Purchase code is not valid or already used.')));
                exit();
            }
        } else {
            echo json_encode(array('status' => '0', 'message' => lang('Something went wrong. cURL is not working.')));
            exit();
        }
    }
    ...
    protected function check_addon_data($addon_data = array())
    {
        if (!isset($addon_data['unique_name']) || $addon_data['unique_name'] == "") {
            echo json_encode(array('status' => '0', 'message' => lang('Add-on unique name has not been provided.')));
            exit();
        }

        if (!$this->is_unique_check("addon_check", $addon_data['unique_name']))  //  unique name must be unique
        {
            echo json_encode(array('status' => '0', 'message' => lang('Add-on is already active. Duplicate unique name found.')));
            exit();
        }
    }
    ...
    protected function register_addon($addon_controller_name = "", $sidebar = array(), $sql = array(), $purchase_code = "", $default_module_name = "")
    {
        ...
        if ($this->session->get('user_type') != 'Admin') {
            echo json_encode(array('status' => '0', 'message' => lang('Access Forbidden')));
            exit();
        }
        ...
        if ($addon_controller_name == "") {
            echo json_encode(array('status' => '0', 'message' => lang('Add-on controller has not been provided.')));
            exit();
        }
        ...
        try {
                        ...
                        if (!empty($child_info)) {
                            ...
                            if (!empty($child))
                                foreach ($child as $key2 => $value2) {
                                    ...
                                    $sub_parent_id = $this->db->insertID();
            ...
            if ($this->db->trans_status() === FALSE) {
                ...
                echo json_encode(array('status' => '1', 'message' => lang('Add-on has been activated successfully.')));
            }
        } //end of try
        catch (Exception $e) {
            $error = $e->getMessage();
    ...
    protected function unregister_addon($addon_controller_name = "")
    {
        ...
        try {
            ...
            if ($this->db->trans_status() === FALSE) {
                ...
                echo json_encode(array('status' => '1', 'message' => lang('Add-on has been deactivated successfully.')));
            }
        } catch (Exception $e) {
            $error = $e->getMessage();
            echo json_encode(array('status' => '0', 'message' => lang($error)));
    ...
    protected function delete_addon($addon_controller_name = "", $sql = array())
    {
        ...
        if (in_array($hostname, ['localhost', 'chatpion.test'])) {
            echo json_encode(array('status' => '0', 'message' => lang('Addon can not be deleted from localhost.')));
            exit();
        }
        ...
        try {
            ...
            if ($this->db->trans_status() === FALSE) {
                ...
                if ($purchase_code != "") {
                    ...
                    $only_domain = get_domain_only(site_url());
                    $url = "https://xeroneit.net/development/envato_license_activation/delete_purchase_code.php?purchase_code={$purchase_code}&domain={$only_domain}&item_name=XeroChat-{$item_name}";
                    $credentials = $this->get_general_content_with_checking($url);
                    $response = json_decode($credentials, true);
                ...
                echo json_encode(array('status' => '1', 'message' => lang('add-on has been deleted successfully.')));
            }
        } catch (Exception $e) {
            $error = $e->getMessage();
            echo json_encode(array('status' => '0', 'message' => lang($error)));
    ...
    {
        if ($this->session->get('user_type') == 'Admin') exit();
        if (!isset($_POST)) exit();
        ...
        if ($this->db->trans_status() === FALSE) {
            echo lang('Something went wrong, please try again.');
        } else {
            $this->session->sess_destroy();
            echo 'success';
        }
    ...
    public function central_webhook_callback()
    {
        ...
        if (($this->config->central_webhook_verify_token ?? null) != '') {
            if ($verify_token === ($this->config->central_webhook_verify_token ?? null)) {
                echo $challenge;
                die();
            }
        }
     ...
     */
    public function data_deletion_callback()
    {
        // Set content type to JSON
        header('Content-Type: application/json');

        try {
            // Get the signed request from POST data
            $signed_request = $this->input->post('signed_request');
            if (is_string($signed_request)) $signed_request = trim($signed_request);
            // Normalize to two segments: signature.payload
            if (!empty($signed_request)) {
                $srParts = explode('.', $signed_request);
                if (count($srParts) > 2) {
                    $signed_request = $srParts[0] . '.' . $srParts[1];
                }
            }
            if (empty($signed_request)) {
            ...
            if (!$data) {
                // Log the failed deletion request
                $this->log_deletion_request('unknown', 'failed', 'Invalid signed_request');
            ...
            if (!$user_id) {
                http_response_code(400);
                echo json_encode([
                    'error' => 'User ID not found in signed_request'
                ]);
                return;
            }

            // Generate confirmation URL first
            $confirmation_code = md5($user_id . time());
            $confirmation_url = base_url('home/data_deletion_status/' . $confirmation_code);

            // Send success response to Facebook immediately
            $this->log_deletion_request($user_id, $confirmation_code, 'processing');
            echo json_encode([
                'url' => $confirmation_url,
                'confirmation_code' => $confirmation_code
            ]);

            // Now perform the actual deletion in the background
            $deletion_result = $this->delete_user_data($user_id);
            $this->log_deletion_request($user_id, $confirmation_code, $deletion_result ? 'success' : 'failed');
        } catch (Exception $e) {
    ...
    {
        try {
            // Get Facebook app config from database
            $facebook_config = $this->basic->get_data("facebook_rx_config", array("where" => array("status" => "1")), "", "", 1);
            ...
            // Load Facebook SDK classes
            require_once ROOTPATH . 'app/libraries/Facebook/autoload.php';

            // Create FacebookApp instance
            $fb_app = new Facebook\FacebookApp($app_id, $app_secret);

            // Create SignedRequest instance
            $signed_request_obj = new Facebook\SignedRequest($fb_app, $signed_request);
            // Get the parsed payload
            $data = $signed_request_obj->getPayload();
            return $data;
        } catch (\Facebook\Exceptions\FacebookSDKException $e) {
            log_message('error', 'Error parsing signed request (FacebookSDKException): ' . $e->getMessage());
            return false;
        } catch (Exception $e) {
            log_message('error', 'Error parsing signed request: ' . $e->getMessage());
            return false;
        }
    }
     ...
     * @return bool
     */
    private function delete_user_data($facebook_user_id = "")
    {
        $facebook_rx_fb_user_info_id = $facebook_user_id;
        $account_information = $this->basic->get_data('facebook_rx_fb_user_info', array('where' => array('fb_id' => $facebook_rx_fb_user_info_id)));
        if (empty($account_information)) {
            return false;
        }

        foreach ($account_information as $value) {
            ...
            $response = $this->delete_data_basedon_account($facebook_rx_fb_user_info_id, 0, 1);
        }
        return true;
    }

    /**
     * Log deletion request for tracking
     * @param string $facebook_user_id
     * @param string $confirmation_code
     * @param string $result
     */
    private function log_deletion_request($facebook_user_id, $confirmation_code, $result)
    {
        try {
            $log_data = array(
            ...
            $this->db->query("CREATE TABLE IF NOT EXISTS `data_deletion_logs` (
            ...
            // Check if record exists with same confirmation code
            $existing_record = $this->db->table('data_deletion_logs')
     ...
     * @param string $confirmation_code
     */
    public function data_deletion_status($confirmation_code = '')
    {
        ...
        $deletion_record = $this->db->table('data_deletion_logs')
            ->where('confirmation_code', $confirmation_code)
            ->get()
            ->getRowArray();
        ...
        if (!empty($deletion_record)) {
            $status = $deletion_record['result'];
        ...
        $data = array(
            ...
            'confirmation_code' => $confirmation_code,
            'status' => $status,
            'show_processing_hint' => $show_processing_hint
        );

        // Load the view
        return view('data_deletion_status', $data);
    }
    ...
    public function sender_action($sender_id, $action_type, $post_access_token = '')
    {
        ...
        $url = "https://graph.facebook.com/v20.0/me/messages?access_token={$post_access_token}";
    ...
    public function redirect_rx_link()
    {
        ...
        $id = $this->session->get("fb_rx_login_database_id");
        ...
        if (isset($user_info['status']) && $user_info['status'] == '0') {
            $data['error'] = 1;
            $data['message'] = lang("Something went wrong") . " : " . $user_info['message'];
            ...
            if ($this->basic->update_data('facebook_rx_config', $where, $update_data)) {
                ...
                if (isset($page_list['error']) && $page_list['error'] == '1') {
                    $data['error'] = 1;
                    $data['message'] = lang("Something went wrong") . " : " . $page_list['message'];
                    $data['body'] = "facebook_rx/admin_login";
                    $this->_viewcontroller($data);
                    exit();
                }
                ...
                $data['error'] = 1;
                $data['message'] = lang("Something went wrong, please try again.");
                $data['body'] = "facebook_rx/admin_login";
                $this->_viewcontroller($data);
            }
        }
    ...
    protected function csrf_token_check($return_bool = false)
    {
        ...
        $csrf_token_session = $this->session->get('csrf_token_session');
        $ajax_resposne = json_encode(array("status" => "0", "message" => lang("CSRF Token Mismatch!"), "error" => lang("CSRF Token Mismatch!")));
        $is_error = false;
        ...
        if ($is_error) {
            ...
            else return redirect()->to('home/error_csrf');
            exit();
        }
        return true;
    }
    ...
    {
        if ($this->session->get('user_type') == 'Admin') return true;
        if ($this->session->get('user_type') == 'Member' && in_array(199, $this->module_access)) return true;
        return false;
    }

    protected function broadcaster_exist()
    {
        if ($this->session->get('user_type') == 'Admin' && $this->basic->is_exist("add_ons", array("project_id" => 30))) return true;
        if ($this->session->get('user_type') == 'Member' && in_array(211, $this->module_access)) return true;
        return false;
    }

    protected function recurring_notification_exist()
    {
        if ($this->session->get('user_type') == 'Admin' && $this->basic->is_exist("add_ons", array("project_id" => 66))) return true;
        if ($this->session->get('user_type') == 'Member' && in_array(335, $this->module_access)) return true;
        return false;
    }

    protected function ai_reply_exist()
    {
        if ($this->session->get('user_type') == 'Admin' && $this->basic->is_exist("add_ons", array("project_id" => 67))) return true;
        if ($this->session->get('user_type') == 'Member' && in_array(340, $this->module_access)) return true;
        return false;
    }
    ...
    {
        if ($this->session->get('user_type') == 'Admin' && $this->basic->is_exist("add_ons", array("project_id" => 30))) return true;
        if ($this->session->get('user_type') == 'Member' && in_array(219, $this->module_access)) return true;
        return false;
    }

    protected function sms_email_drip_campaigner_exist()
    {
        if ($this->session->get('user_type') == 'Admin' && $this->basic->is_exist("add_ons", array("project_id" => 40))) return true;
        if ($this->session->get('user_type') == 'Member' && (in_array(270, $this->module_access) || in_array(271, $this->module_access))) return true;
        return false;
    }
    ...
    {
        if ($this->session->get('user_type') == 'Admin' && $this->basic->is_exist("add_ons", array("project_id" => 31))) return true;
        if ($this->session->get('user_type') == 'Member' && in_array(257, $this->module_access)) return true;
        return false;
    }
    ...
    {
        if ($this->session->get('user_type') == 'Admin') return true;
        if ($this->session->get('user_type') == 'Member' && in_array(260, $this->module_access)) return true;
        return false;
    }
    ...
    {
        if ($this->session->get('user_type') == 'Admin'  && $this->basic->is_exist("add_ons", array("project_id" => 30))) return true;
        if ($this->session->get('user_type') == 'Member' && count(array_intersect($this->module_access, array(213, 214, 215, 217))) > 0) return true;
        return false;
    }
    ...
    {
        if ($this->session->get('user_type') == 'Admin'  && $this->basic->is_exist("add_ons", array("project_id" => 31))) return true;
        if ($this->session->get('user_type') == 'Member' && in_array(261, $this->module_access)) return true;
        return false;
    }
    ...
    {
        if ($this->session->get('user_type') == 'Admin') return true;
        if ($this->session->get('user_type') == 'Member' && in_array(268, $this->module_access)) return true;
        return false;
    }

    protected function auto_social_sharing_exist()
    {
        if ($this->session->get('user_type') == 'Admin'  && $this->basic->is_exist("add_ons", array("project_id" => 39))) return true;

        if ($this->session->get('user_type') == 'Member' && in_array(269, $this->module_access)) return true;
    ...
    public function thirdparty_webhook_trigger($page_id = "", $subscriber_id = "", $trigger = 'trigger_email', $postback_id = "", $form_canonical_id = "", $form_data = array())
    {
        ...
        // Extract keyword message if trigger is trigger_keyword
        $keyword_message = "";
        if (strpos($trigger, 'trigger_keyword_') === 0) {
            $keyword_message = substr($trigger, strlen('trigger_keyword_'));
        }
        ...
        foreach ($webhook_connector_info as $webhook_value) {
            ...
            $post_info = array();
            if ($keyword_message != "") {
                $post_info['message'] = $keyword_message;
            }
    ...
    {
        if ($table_id == 0) {
            return json_encode(array('success' => 0, 'message' => lang("Page is not found for this user. Something is wrong.")));
            exit();
        }

        if ($admin_access == '1' && $this->session->get('user_type') == 'Admin')
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            $response['status'] = 1;
            $response['message'] = lang("Your page and all of it's corresponding campaigns have been deleted successfully.");
        }
    ...
    public function delete_data_basedon_account($fb_user_id = 0, $app_delete = 0, $from_webhook = 0)
    {
        $this->db->transStart();
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            $response['status'] = 1;
            $response['message'] = lang("Your account and all of it's corresponding pages, groups and campaigns have been deleted successfully. Now you'll be redirected to the login page.");
        }
        return $response;
    }
    ...
    public function user_delete_action($user_id = 0)
    {
        ...
        if ($this->is_demo == '1' && $this->session->get('user_type') == "Admin") {
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            if ($this->session->get('user_type') != 'Admin')
                $this->session->sess_destroy();
            $response['status'] = 1;
            $response['message'] = lang("Account and all of it's corresponding pages, groups and campaigns have been deleted successfully.");
        }
        echo json_encode($response);
    }
    ...
    public function unsubscribe($contact_id, $email)
    {
        ...
        if (isset($info) && !empty($info)) {
            ...
            return view("sms_email_manager/email/email_campaign/unsubscribed_message", $data);
        } else {
            return redirect()->to('home/access_forbidden');
        }
    }
    ...
    public function common_get_postback()
    {
        ...
        if ($is_from_add_button == '0') {
            $push_postback .= "<option value=''>" . lang("Select") . "</option>";
        }
    ...
    public function switch_to_page()
    {
        ...
        $this->session->set('selected_global_page_table_id', $page_id);
    ...
    protected function updated_subscriber_last_interaction($subscriber_id = null, $update_interaction = false, $message_content = null, $last_conversation_message = null, $update_unseen = true)
    {
        if (!empty($subscriber_id) && $subscriber_id != 0) {
            ...
            $curtime = convert_datetime_to_timezone($curtime, 'UTC', '', 'Y-m-d H:i:s', ($this->config->time_zone ?? null));
    ...
    protected function insert_livechat_data($insert_data = [], $check_access = true)
    {
        ...
        if (empty($user_id) || empty($page_id)) {
            $page_info = $this->basic->get_data("facebook_rx_fb_page_info", ["page_id" => $fb_page_id, 'user_id' => $user_id], ["id", "user_id"]);
        ...
        if ($insert_data['sender'] == 'bot' && $this->session->get('logged_in') == 1) $insert_data['agent_name'] = $this->session->get('username');
        ...
        if ($has_livechat_access) {
            ...
            $currtime = convert_datetime_to_timezone($currtime, 'UTC', '', 'Y-m-d H:i:s', ($this->config->time_zone ?? null));
            // dd($currtime);
    ...
    protected function http_api_trigger($settings_http_api_id = null, $subscriber_unique_id = null, $subscriber_unique_field = 'subscribe_id', $settings_http_api_data = null, $sandbox = false)
    { // accepts both id and subscriber_id, settings_http_api_data used in sandbox
        $this->load->helper('http_api/http_api');
        if ((empty($settings_http_api_id) && empty($settings_http_api_data)) || empty($subscriber_unique_id)) {
            return $sandbox ? ["data_error" => lang("HTTP API or Subscriber ID not provided.")] : false;
        }

        if (empty($settings_http_api_data)) {
            $builder = $this->db->table('settings_http_apis')->where('id', $settings_http_api_id);
            if (!$sandbox) $builder->where('status', '1');
            $settings_http_api_data = $builder->get()->getRow();
        }

        if (empty($settings_http_api_data)) {
            return $sandbox ? ["data_error" => lang("HTTP API connection not found.")] : false;
        }
        ...
        $subscriber_data = $this->db->table($subscriber_table_name)
            ->where($subscriber_unique_field, $subscriber_unique_id)
            ->get()
            ->getRow();
        if (empty($subscriber_data)) {
            return $sandbox ? ["data_error" => lang("Subscriber not found.")] : false;
        }
        if ($subscriber_data->user_id != $user_id) {
            return $sandbox ? ["data_error" => lang("Subscriber does not belong to the HTTP API.")] : false;
        }
        ...
        $subscriber_custom_field_data = $this->db->table($custom_field_table_name)
            ->where($custom_field_map_bot_id_field, $bot_id)
        ...
        $custom_field_data = $this->db->table($custom_field_table_name)
            ->select("id, name")
            ->where("user_id", $user_id)
            ->get()
            ->getResult();
        ...
        $formatter_data = $this->db->table('settings_http_api_formatters')
            ->where('settings_http_api_id', $settings_http_api_id)
            ->get()
            ->getResult();
        ...
        try {
            ...
            if (!empty($update_subscriber_data)) {
                $this->db->table($subscriber_table_name)
                    ->where("id", $subscriber_auto_id)
                    ->update($update_subscriber_data);
            }
            ...
            if (count($update_subscriber_custom_field_data) > 0 && !empty($custom_field_ids)) {
                ...
                foreach ($update_subscriber_custom_field_data as $row) {
                    ...
                    $this->db->select("id");
                    $exist = $this->db->table($custom_field_map_table_name)
                        ->where($check_where)
                        ->get()
                        ->getRow();
                    ...
                    if (!empty($exist)) {
                        $update_id = $exist->id;
                        $this->db->table($custom_field_map_table_name)
                            ->where("id", $update_id)
                            ->update([
