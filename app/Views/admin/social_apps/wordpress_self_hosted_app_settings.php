...
<section class="section section_custom">
	<div class="section-header">
		<h1><i class="fab fa-wordpress"></i> <?php echo lang('Wordpress Settings (Self-Hosted)'); ?></h1>
		<div class="section-header-button">
	     	<a class="btn btn-primary" href="<?= base_url('social_apps/add_wordpress_self_hosted_settings') ?>">
	        <i class="fas fa-plus-circle"></i> <?php echo lang('Add New Site'); ?></a>
	    </div>
	...
	<div class="section-body">
		<div class="row">
			<div class="col-12">
				...
				<?php if (session()->get('edit_wssh_success')): ?>
				<div class="alert alert-success alert-dismissible show fade">
					<div class="alert-body text-center">
						<button class="close" data-dismiss="alert">
							<span>×</span>
						</button>
						<?php echo session()->get('edit_wssh_success'); ?>
					</div>
				</div>
				<?php session()->remove('edit_wssh_success'); ?>
				<?php endif; ?>

				<div class="card">
					<div class="card-header d-flex justify-content-between">
						<p><?php echo lang('Make sure the REST API is NOT disabled of your wordpress blog.'); ?></p>
						<a class="btn btn-primary" href="<?php echo base_url('assets/wordpress-self-hosted/wp-self-hosted-poster.zip'); ?>"><i class="fa fa-download"></i> <?php echo lang('Download API Plugin'); ?></a>
					</div>
							...
							<table id="wssh-datatable" class="table table-bordered" style="width:100%">
						        <thead>
						            <tr>
						                ...
						                <th><?php echo lang('User Key'); ?></th>
						                <th><?php echo lang('Authentication Key'); ?></th>
	...
	$(document).ready(function() {
		...
		$(document).on('click', '.update-categories', function(e) {
			...
			$.ajax({
				...
				success: function(res) {
					...
					if (false === res.status) {
						swal('<?php echo lang('Error'); ?>', res.message, 'error');
						return;
					}

					if (true === res.status) {
						swal('<?php echo lang('Success'); ?>', res.message, 'success');
					}
				},
			});
		});
		...
		$(document).on('click', '#delete-wssh-settings', function(e) {
			...
			swal({
				...
				text: '<?php echo lang('Once deleted, you will not be able to recover this wordpress site settings!'); ?>',
				icon: 'warning',
				buttons: true,
				dangerMode: true,
			}).then((yes) => {
				if (yes) {
					$.ajax({
						...
						success: function(res) {
							...
							if ('ok' == res.status) {
								swal('<?php echo lang('Success'); ?>', res.message, 'success').then((value) => {
								    location.reload();
								});
							} 
							else swal('<?php echo lang('Error'); ?>', res.error, 'error');
