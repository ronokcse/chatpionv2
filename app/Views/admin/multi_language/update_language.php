	...
	<div class="section-body">
		...
		{ ?>
			<div class="card">
		            ...
		            <div class="form-group">
			            <div class="input-group mb-3" id="languagename_field">
			                ...
			                <div class="input-group-append">
			                  <button class="btn btn-primary" type="submit" id="update_language_name"><i class="fas fa-save"></i> <?php echo lang('Update Language'); ?></button>
			                </div>
			            </div>
		            </div>
		        <?php 
	          	...
	          	{ ?>
	          		...
	          		<div class="not_english text-center alert alert-warning">
	          			<?php echo lang('English language name can not be updated. You Can update the content if you like.'); ?>
	          		</div>
	          	<?php 
	            } ?>
	          </div>
	          ...
	          <div class="card-header">
	          	<h4 class="text-center" style="width: 100%"><?php echo lang('System Languages')." : ".$languagename." (".count($folderFiles)." ".lang('files').")"; ?></h4>
	          </div>
		...
		{ ?>
			<div class="card">
	          ...
	          <div class="card-header">
	          	<h4 class="text-center" style="width: 100%"><?php echo lang('3rd Party Languages')." : ".$plugin_file; ?></h4>
	          </div>
...
<!-- 			<div class="card">
	          ...
	          <div class="card-header">
	          	<h4 class="text-center" style="width: 100%"><?php echo ucfirst(str_replace("_",' ',$languagename))." ".lang('Add-on Languages')." (".count($module_language_folders)." ".lang('files').")"; ?></h4>
	          </div>
	          ...
	          <div class="card-body">
	          	...
	          	{ ?>
          			<div class="text-center alert alert-warning">
	          			<?php echo lang('This language folder is empty. No files to show'); ?>
	          		</div>
	          	<?php
	          	}?>
	          </div> 
...
<script>
	...
	$(document).ready(function(){
		...
		$(document).on('click', '#update_language_name', function(event) {
			...
			{
				swal('<?php echo lang('Warning')?>', '<?php echo lang('This language already exist, no need to update.') ?>', 'warning');
			...
			else 
			{
				$.ajax({
					...
					success:function(response)
					{
						...
						{
							swal('<?php echo lang('Error') ?>', '<?php echo lang('This language is already exist, please try with different one.') ?>', 'error');
						}
					}
				});
			}
		...
		$(document).on('click', '.allFiles', function(event) {
		    ...
		    $.ajax({
		    	...
		    	success:function(response)
		    	{
			    	...
			    	{
			    		swal('<?php echo lang('Error') ?>', '<?php echo lang('Your given name has not updated, please update the name first.') ?>', 'error');
			    	} else
			    	{
			    		$('#response_status').html(loading);
			    	}
  ...
  <div class="modal-dialog modal-lg" role="document" style="min-width: 90%">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title"><i class="fas fa-edit"></i> <?php echo lang('Edit Language Translation');?></h5>
      ...
      <div class="modal-footer bg-whitesmoke br">
        <button form_id="language_creating_form" class="btn btn-primary btn-lg update_language_button"><i class="fas fa-save" aria-hidden="true"></i>  <?php echo lang('Save'); ?> </button>
