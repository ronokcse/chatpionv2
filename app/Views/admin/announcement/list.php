<section class="section">
    ...
    { ?>
      <div class="section-header-button">
       <a class="btn btn-primary"  href="<?php echo site_url('announcement/add');?>">
          <i class="fas fa-plus-circle"></i> <?php echo lang('New Announcement'); ?>
       </a> 
      </div>
    <?php 
    } ?>
  ...
  <?php 
  if(session()->getFlashdata('mark_seen_success')!='')
  echo "<div class='alert alert-success text-center'><i class='fas fa-check-circle'></i> ".session()->getFlashdata('mark_seen_success')."</div>"; 
  ?>
        ...
        <div class="input-group mb-3" id="searchbox">
          <div class="input-group-prepend">
              <select class="select2 form-control" id="seen_type">
                <option value="0"><?php echo lang('Unseen'); ?></option>
                <option value="1"><?php echo lang('Seen'); ?></option>
                <option value=""><?php echo lang('Everything'); ?></option>
              </select>
            </div>
          <input type="text" class="form-control" id="search" autofocus placeholder="<?php echo lang('Search...'); ?>" aria-label="" aria-describedby="basic-addon2">
          <div class="input-group-append">
            <button class="btn btn-primary" id="search_submit" type="button"><i class="fas fa-search"></i> <?php echo lang('Search'); ?></button>
          </div>
        </div>
      </div>
      <div class="col-12 col-md-5">
        <button class="btn btn-outline-primary btn-lg float-right" id="mark_seen_all"><i class="fas fa-eye-slash"></i> <?php echo lang('Mark all unseen as seen'); ?></button>
      </div>
    </div>
    ...
    <div class="card" id="nodata" style="display: none">
      <div class="card-body">
        <div class="empty-state">
          ...
          <h2 class="mt-0"><?php echo lang('We could not find any data.') ?></h2>
        </div>
      </div>
    </div>
 

    <button class="btn btn-outline-primary float-right" style="display: none;" id="load_more" data-limit="10" data-start="0"><i class="fas fa-book-reader"></i> <?php echo lang('Load More'); ?></button>
...
<script>       
    ...
    $(document).ready(function() {      
            ...
            data: {start:start,limit:limit,search:search,seen_type:seen_type},
              success:function(response)
              {
                ...
                else 
                {
                  ...
                  if(popmessage) 
                  {
                    swal("<?php echo lang('No data found') ?>", "", "warning");
                    $("#nodata").hide();
                  }
                  else $("#nodata").show();
                }
          ...
          $.ajax({
            ...
            data: {},
              success:function(response)
              {
                ...
                {
                  iziToast.success({title: '<?php echo lang('Success'); ?>',message: response.message,position: 'bottomRight'});
        ...
        $(document).on('click', '#mark_seen_all', function(e) {
            e.preventDefault();
            var mes='<?php echo lang('Do you really want to mark all unseen notifications as seen?');?>';  
            swal({
