<section class="section section_custom">
  <div class="section-header">
    ...
    <!-- <div class="section-header-breadcrumb">
      <div class="breadcrumb-item"><a href="<?php echo base_url('subscriber_manager'); ?>"><?php echo lang('Subscriber Manager');?></a></div>
                      ...
                      <div class="col-12 col-md-10 mr-0">
                        ...
                        '<div class="input-group" id="searchbox">
                          ...
                          <select class="form-control select2" id="email_phone_birth" name="email_phone_birth[]" multiple="multiple" style="max-width:30%;">
                            <option value="has_phone">'.lang('Has Phone').'</option>
                            <option value="has_email">'.lang('Has Email').'</option>
                            <option value="has_birthdate">'.lang('Has Birth Date').'</option>
                          </select>

                          <input type="text" class="form-control" autofocus id="search_value" name="search_value" placeholder="'.lang('Search...').'" style="max-width:30%;">
                          <div class="input-group-append">
                            <button class="btn btn-primary" type="button" id="search_subscriber"><i class="fas fa-search"></i> <span class="d-none d-sm-inline">'.lang('Search').'</span></button>
                          </div>
                        </div>'; ?>                                          
                      </div>
                      ...
                      <div class="col-12 col-md-2">
                        ...
                        <div class="btn-group dropleft float-right mr-1">
                          ...
                          <div class="dropdown-menu dropleft" style="width:auto !important">
                            <a class="dropdown-item" href="<?php echo base_url('subscriber_manager/download_result'); ?>"><i class="fas fa-cloud-download-alt"></i> <?php echo lang('Download search result'); ?></a>
                            
                            <div class="dropdown-divider"></div>
                            <a class="dropdown-item" id="assign_group" href=""><i class="fas fa-user-tag"></i> <?php echo lang('Assign labels to selected subscribers'); ?></a>
                            <div class="dropdown-divider"></div>
                            <a class="dropdown-item red" id="bulk_delete_contact" href=""><i class="fas fa-trash"></i> <?php echo lang('Delete Subscriber'); ?></a>
                          </div>
                        </div>                          
                      </div>
                    </div>
                        ...
                        <table class="table table-bordered" id="mytable">
                          <thead>
                            <tr>
                              ...
                              <th><?php echo addon_exist($module_id=320,$addon_unique_name="instagram_bot") ? lang('Page/Account') : lang('Page Name'); ?></th>    
                              ...
                              <th><?php echo lang('Quick Info'); ?></th>      
                              <th><?php echo lang('Label/Tag'); ?></th>      
                              <th><?php echo lang('Thread ID'); ?></th>      
                              <th><?php echo lang('Synced at'); ?></th>
                              <th><?php echo lang('Social Media'); ?></th>
                            </tr>
                          </thead>
                        </table>
                    </div>
                      ...
                      <div class="row">                
                          <div class="col-12">
                              <a class="btn btn-outline-primary add_label btn-block btn-lg mb-1"  href="#">
                                <i class="fas fa-plus-circle"></i> <?php echo lang('New Label'); ?>
                              </a> 
                          </div>
                          <div class="col-12">
                              <div class="input-group float-left" id="searchbox">
                                  <input type="text" class="form-control" id="searching" name="searching" autofocus placeholder="<?php echo lang('Search Labels...'); ?>" aria-label="" aria-describedby="basic-addon2">
                              </div>
                          </div>
                      </div>
                      <div class="table-responsive2">
...
<?php     
    $doyouwanttodeletethiscontact = lang('Do you want to delete this subscriber?');
    $youhavenotselected = lang('You have not selected any subscriber to assign label. You can choose upto');
    $youhavenotselectanysubscribertoassignsequence = lang('You have not selected any subscriber to assign sms/email sequence campaign. You can choose upto');
    $youhavenotselected2 = lang('You have not selected any subscriber to delete.');
    $leadsatatime = lang('subscribers at a time.');
    $youcanselectupto = lang('You can select upto');
    $leadsyouhaveselected = lang(',you have selected');
    $leads = lang('subscribers.');
    $youhavenotselectedany = lang('You have not selected any subscriber to delete. you can choose upto');
    $youhavenotselectedanyleadtoassigngroup = lang('You have not selected any subscriber to assign label.');
    $youhavenotselectedanyleadtoassigndripcampaign = lang('You have not selected any subscriber to assign sequence campaign.');
    $youhavenotselectedanyleadgroup = lang('You have not selected any label.');
    $youhavenotselectedanysequence = lang('You have not selected any sequence campaign.');
    $pleasewait = lang('Please wait...');
    $groupshavebeenassignedsuccessfully = lang('Labels have been assigned successfully');
    $sequencehavebeenassignedsuccessfully = lang('Sequence campaign have been assigned successfully');
    $contactshavebeendeletedsuccessfully = lang('Subscribers have been deleted successfully');
    $somethingwentwrongpleasetryagain = lang('Something went wrong, please try again.'); 
    $ig_bot_exists = addon_exist($module_id=320,$addon_unique_name="instagram_bot") ? '1' : '0';   

    $disabledsuccessfully = lang('Backgound scanning has been disabled successfully.');
    $enabledsuccessfully = lang('Backgound scanning has been enabled successfully.');
?>
...
<script type="text/javascript">
    ...
    $("document").ready(function(){
      ...
      $(document).on('click','#assign_group',function(e){
          ...
          if(selected_page=="")
          {
            swal('<?php echo lang('Warning') ?>',"<?php echo lang('To assign labels in bulk you have to search by any page first.');?>", 'warning');
              return;
          }

          if(ids=="") 
          {
            swal('<?php echo lang('Warning') ?>', youhavenotselected+" "+upto+" "+leadsatatime, 'warning');
            return;
          } 
          if(selected>upto) 
          {
              swal('<?php echo lang('Warning') ?>',youcanselectupto+" "+upto+" "+leadsyouhaveselected+" "+selected+" "+leads, 'warning');
              return;
          }
      ...
      $(document).on('click', '#assign_sms_email_sequence', function(event) {
        ...
        if(selected_page=="")
        {
          swal('<?php echo lang('Warning') ?>',"<?php echo lang('To assign sequence in bulk you have to search by any page first.');?>", 'warning');
            return;
        }

        if(ids=="") 
        {
          swal('<?php echo lang('Warning') ?>', youhavenotselectanysubscribertoassignsequence+" "+upto+" "+leadsatatime, 'warning');
          return;
        } 
        if(selected>upto) 
        {
      ...
      $(document).on('click','#assign_group_submit',function(e){
          e.preventDefault();
          swal({
             title: '<?php echo lang('Assign Label'); ?>',
             text: '<?php echo lang('Do you really want to assign selected labels to your selected subscribers? Please be noted that bulk assigning labels will replace subscribers previous labels if any.'); ?>',
             icon: 'warning',
             buttons: true,
             dangerMode: true,
           })
           .then((willDelete) => {
             if (willDelete) 
             {
                 ...
                 if(count==0) 
                 {
                   swal('<?php echo lang('Error') ?>', youhavenotselectedanyleadgroup, 'error');
                   return;
                 } 
                 ...
                 $.ajax({
                   ...
                   success:function(response)
                   {
                    ...
                    if(response=="1"){                      
                      swal('<?php echo lang('Label Assign') ?>', groupshavebeenassignedsuccessfully+" ("+selected+")", 'success')
                      .then((value) => {
                      ...
                      swal('<?php echo lang('Label Assign') ?>',"<?php echo lang('You do not have access to perform this action.');?>", 'error');
                    }
      ...
      $(document).on('click','#assign_sequence_submit',function(e){
        ...
        if(count==0) 
        {
          swal('<?php echo lang('Error') ?>', youhavenotselectedanysequence, 'error');
          return;
        } 
        ...
        $.ajax({
          ...
          success:function(response)
          {
            ...
            swal('<?php echo lang('Sequence Campaign Assign') ?>', sequencehavebeenassignedsuccessfully+" ("+selected+")", 'success')
            .then((value) => {
      ...
      $(document).on('click','#bulk_delete_contact',function(e){
          ...
          if(page_id=="")
          {
            swal('<?php echo lang('Warning') ?>',"<?php echo lang('To delete subscribers in bulk you have to search by any page first.');?>", 'warning');
              return;
          }     
          if(ids=="") 
          {
            swal('<?php echo lang('Warning') ?>', youhavenotselected2, 'warning');
            return;
          } 

          swal({
             title: '<?php echo lang('Delete Subscribers'); ?>',
             text: '<?php echo lang('Do you really want to delete selected subscribers?'); ?>',
             icon: 'error',
             buttons: true,
             dangerMode: true,
           })
           .then((willDelete) => {
             if (willDelete) 
             {
                 $.ajax({
                   ...
                   {
                    if(response=='success'){                      
                      swal('<?php echo lang('Delete Subscribers') ?>', contactshavebeendeletedsuccessfully+" ("+selected+")", 'success')
                      .then((value) => {                   
                       table1.draw(false);
                      });
                    }
                    else{
                      swal('<?php echo lang('Delete Subscribers') ?>', '<?php echo lang('You do not have access to perform this action.') ?>', 'error')
                      .then((value) => {                   
                       table1.draw(false);
                      });
                    }
    ...
    $(document).on('click', '#create_label_main', function(event) {
        ...
        // if(selected_page_id == '') {
        //     $("#page_err").text("<?php echo lang('Page is Required') ?>")
        //     return false;
        // }
    ...
    $(document).on('click', '.delete_label', function(event) {
        ...
        swal({
            title: '<?php echo lang('Delete Label'); ?>',
            text: '<?php echo lang('Do you want to delete this label?'); ?>',
            icon: 'warning',
            buttons: true,
            dangerMode: true,
        })
  ...
  $("document").ready(function(){
    setTimeout(function(){ 
    var session_value = "<?php echo session()->get('sync_subscribers_get_page_details_page_table_id'); ?>";
    var elem;
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title"><i class="fas fa-user-tag"></i> <?php echo lang('Assign Label');?></h5>
            ...
            <div class="modal-footer">
              <a class="btn btn-primary float-left" href="" id="assign_group_submit"><i class="fas fa-user-tag"></i> <?php echo lang('Assign Label') ?></a>
              <a class="btn btn-outline-secondary float-right" data-dismiss="modal"><i class="fas fa-times"></i> <?php echo lang('Close') ?></a>
            </div>
        </div>
    </div>
</div>
    ...
    <div class="modal-dialog" style='min-width:40%;'>
        <div class="modal-content">
            <div class="modal-header bbw">
              <h5 class="modal-title"><i class="fas fa-sort-numeric-up"></i> <?php echo lang('Assign sms/email Sequence');?></h5>
              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">×</span>
              </button>
            </div>
            ...
            <div class="modal-footer bg-whitesmoke">
              <a class="btn btn-lg btn-primary float-left" href="" id="assign_sequence_submit"><i class="fas fa-save"></i> <?php echo lang('Assign Sequence') ?></a>
              <a class="btn btn-lg btn-light float-right" data-dismiss="modal"><i class="fas fa-times"></i> <?php echo lang('Close') ?></a>
            </div>
        </div>
    </div>
</div>
    ...
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header" style="padding:15px;">
              <h5 class="modal-title"><i class="fas fa-user-circle"></i> <?php echo lang('Subscriber Actions');?></h5>
              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">×</span>
              </button>
            </div>
            ...
            <div class="modal-body" id="subscriber_actions_modal_body" style="padding:0 15px 15px 15px;" data-backdrop="static" data-keyboard="false">
              ...
              <ul class="nav nav-tabs" id="myTab" role="tablist">
                <li class="nav-item">
                  <a class="nav-link active" id="default-tab" data-toggle="tab" href="#default" role="tab" aria-controls="default" aria-selected="true"><?php echo lang('Subscriber Data'); ?></a>
                </li>

                <?php if($user_input_flow_exist == 'yes') : ?>
                <li class="nav-item">
                  <a class="nav-link" id="flowanswers-tab" data-toggle="tab" href="#flowanswers" role="tab" aria-controls="flowanswers" aria-selected="false"><?php echo lang('User Input Flow Answer'); ?></a>
                </li>
                <li class="nav-item">
                  <a class="nav-link" id="customfields-tab" data-toggle="tab" href="#customfields" role="tab" aria-controls="customfields" aria-selected="false"><?php echo lang('Custom Fields'); ?></a>
                </li>
                <?php endif; ?>

                <?php if($webview_access == 'yes') : ?>
                <li class="nav-item">
                  <a class="nav-link" id="formdata-tab" data-toggle="tab" href="#formdata" role="tab" aria-controls="formdata" aria-selected="false"><?php echo lang('Custom Form Data'); ?></a>
                </li>
                <?php endif; ?>
                <?php if($ecommerce_exist == 'yes') : ?>
                <li class="nav-item">
                  <a class="nav-link" id="purchase-tab" data-toggle="tab" href="#purchase" role="tab" aria-controls="purchase" aria-selected="false"><?php echo lang('Purchase History'); ?></a>
                </li>
                <?php endif; ?>
                      ...
                      <div class="row">
                        <div class="col-12 col-md-9">
                          ...
                          '<div class="input-group mb-3" id="searchbox">
                            ...
                            <input type="text" class="form-control" id="search_value2" autofocus name="search_value2" placeholder="'.lang('Search...').'" style="max-width:25%;">
                            <div class="input-group-append">
                        ...
                        <?php
                          echo $drop_menu ='<a href="javascript:;" id="search_date_range" class="btn btn-primary btn-lg float-right icon-left btn-icon"><i class="fas fa-calendar"></i> '.lang('Choose Date').'</a><input type="hidden" id="search_date_range_val">';
                        ?>
                          ...
                          <table class="table table-bordered" id="mytable2">
                            <thead>
                              <tr>
                                ...
                                <th style="max-width: 130px"><?php echo lang('Status')?></th>              
                                ...
                                <th><?php echo lang('Docs')?></th>                              
                                <th><?php echo lang('Ordered at')?></th>                   
...
<div class="modal fade" id="add_label" data-backdrop="static" data-keyboard="false">
    <div class="modal-dialog" style="min-width: 30%;">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><i class="fa fa-plus-circle"></i> <?php echo lang('Add Label') ?></h5>
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="form-group">
                          <label><i class="fas fa-tags"></i> <?php echo lang('Label Name'); ?></label>
            ...
            <div class="modal-footer">
              ...
              <button id="create_label_main" type="button" class="btn btn-lg btn-primary"><i class="fas fa-save"></i> <?php echo lang('Save'); ?></button>
            </div>
        </div>
    </div>
</div>
...
<div class="modal fade" id="subscriber_info_modal" data-backdrop="static" data-keyboard="false">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title"><i class="fas fa-users"></i> <?php echo lang('Subscribers');?></h5>
              <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">×</span>
              </button>
            </div>
            ...
            <div class="modal-body">    
              ...
              <div class="section">                
                <h2 class="section-title"><?php echo lang('24H Subscribers'); ?></h2>
                <p><?php echo lang('Those users who interacted with your messenger bot within 24 hours. This subscribers are eligible to get promotional message through Subscriber Broadcast.'); ?></p>
              </div>
              <div class="section">                
                <h2 class="section-title"><?php echo lang('Unavailable'); ?></h2>
                <p><?php echo lang('You may find red color number as unavailable beside both Conversation Subscribers & BOT Subscribers means the number of users are unavailable for broadcast, because in last broadcasting campaign, Facebook responded with error during sending message to them. They will not be eligible for future broadcast campaign. However once that user send message to your page again, then user become available again.'); ?></p>
              </div>
              <div class="section">                
                <h2 class="section-title"><?php echo lang('Migrated Subscribers'); ?></h2>
                <p><?php echo lang('Those subscribers are migrated as BOT subscribers from Conversation Subscribers. These are basically all old subscribers achieved before using our system for Messenger BOT.'); ?></p>
              </div>
            </div>
