<section class="section section_custom">
  <div class="section-header">
    ...
    <div class="section-header-button">
      <a href="<?php echo base_url('messenger_bot_broadcast/create_conversation_campaign'); ?>" class="btn btn-primary"><i class="fas fa-plus-circle"></i> <?php echo lang('Create Campaign'); ?></a>
    </div>
  ...
  <div class='text-center text-danger' style='padding:12px;border:.5px solid #dee2e6; color:var(--blue);background: #fff;font-size: 20px;'>
    <?php  echo lang('Conversation broadcast feature is going to be deprecated after 30th June,2020 for all old Facebook APPs. For new Facebook APP v6.0, conversation broadcasting feature will not work anymore.'); ?>
  </div>
...
<script>
	...
	$(document).on('click','.unsubscribe_me',function(){
		...
		swal({
			title: '<?php echo lang('Unsubscribe'); ?>',
			...
			if (willDelete) 
			{
			    ...
			    $.ajax({
			       ...
			       success:function(response)
			       {
				      	...
				      	if(response=='1') 
				       	{
				      		iziToast.success({title: '',message: '<?php echo lang('subscriber has been unsubscribed successfully.'); ?>',position: 'bottomRight'});
				      		$(this).hide();
				       	}      	
...
<div class="modal fade" id="error_message_learn" tabindex="-1" role="dialog" aria-labelledby="error_message_learn" aria-hidden="true" data-backdrop="static" data-keyboard="false">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
     ...
     <div class="section">                
        <h2 class="section-title"> '.lang('(#368) the action attempted has been deemed abusive or is otherwise disallowed').'</h2>
        <p>
         '.lang('This error messages comes from Facebook. The possible reasons are below').' : 
         <ol>
              <li>'.lang('Your content may be spammy by Facebook filter.').'</li>
              <li>'.lang(' You have any link attached that blocked by Facebook').'</li>
              <li>'.lang('You are sending message too fast without setting delay.').'</li>
              <li>'.lang('If too much people mark your message as spam.').'</li>
              <li>'.lang('The subscribers from whom it gets the error message is inactive from many days with your messenger.').'</li>
              <li>'.lang('The subscribers who has marked your message as spam in past.').'</li>
        </ol>
        '.lang('In this case system automatically mark the subscriber as unviable for future conversation broadcasting campaign. ').'
        </p>

        <h2 class="section-title"> '.lang('(#230) Permissions disallow message to user').'</h2>
        <p>
        '.lang('This error messages comes from Facebook. The possible reasons are below').' : 
         <ol>
              <li>'.lang('Subscribers block your page from sending message from their settings').'</li>
              <li>'.lang('Subscriber has deactivated their Facebook account').'</li>
        </ol>
        '.lang('In this case system automatically mark the subscriber as unviable for future conversation broadcasting campaign. ').'
        </p>
      </div>
