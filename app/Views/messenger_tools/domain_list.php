    ...
    <table class="table table-bordered table-sm table-striped" id="mytable">
      <thead>
        <tr>
          ...
          <th><?php echo lang('FB Account')?></th>
          <th><?php echo lang('Page Name')?></th>
          <th><?php echo lang('Domain Count')?></th>
...
$drop_menu = '<a href="#" class="float-right btn btn-primary" type="button" id="add_new_domain"><i class="fas fa-plus-circle"></i> '.lang('New Domain').'</a>';
?> 
...
<script>       
    ...
    $(document).ready(function() {
      ...
      $(document).on('click','#add_new_domain_submit',function(){
          ...
          {
             swal('<?php echo lang('Warning'); ?>', "<?php echo lang('Please select page and provide domain name.'); ?>", 'warning');
             return false;
          }
          ...
          $.ajax({
            ...
            { 
              ...
              if(response.status=='1')
                swal('<?php echo lang('Success'); ?>', response.result, 'success').then((value) => {
                  $("#add_new_domain_modal").modal('hide');
                  table.draw();
                });
              else
                swal('<?php echo lang('Error'); ?>', response.result, 'error');
            }
          });
      ...
      $(document).on('click','.delete_domain',function(e){
        ...
        swal({
          ...
          text: '<?php echo lang('Do you want to detete this domain?'); ?>',
          icon: 'warning',
          buttons: true,
          dangerMode: true,
        })
          ...
          if (willDelete) 
          {
            ...
            $.ajax({
              ...
              success:function(response){ 
                ...
                if(response=='1')
                {
                  var deleted = "<?php echo lang('Deleted'); ?>";
                  var deleted_html="<span class='badge badge-light red'><i class='fa fa-check'></i> "+deleted+"</span>";
                  iziToast.success({title: '',message: '<?php echo lang('Domain has been deleted successfully.'); ?>',position: 'bottomRight'});
                  $("#"+col_id).parent().html(deleted_html);
                  table1.draw();
                }
                else
                {
                  swal('<?php echo lang('Error'); ?>', response, 'error');
                }
              }
            });
          } 
    ...
    <div class="modal-dialog  modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><i class="fa fa-list-ol"></i> <?php echo lang('Domain List');?></h5>
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
            </div>
            <div class="modal-body">
                <div class="row">
                    <div class="col-12">
                      <h4 class='text-left'><?php echo lang('Page')." : "; ?><span id="put_page_name"></span> (<span id="put_account_name"></span>)</h4><br/>
                    </div>
                          ...
                          <table class="table table-bordered table-sm table-striped" id="mytable1">
                            <thead>
                              <tr>
                                ...
                                <th><?php echo lang('Whitelisted At'); ?></th> 
...
<div class="modal fade" id="add_new_domain_modal" data-backdrop="static" data-keyboard="false">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><i class="fas fa-plus-circle"></i> <?php echo lang('Add Domain');?></h5>
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
            </div>
            <div class="modal-body">
                ...
                <div class="form-group col-12 hidden" style="padding:0 5px 0 0">
                  ...
                  <select class="form-control select2" id="add_new_domain_page_old" name="add_new_domain_page_old">
                    <?php 
                    echo "<option value=''>".lang('Choose Page')."</option>";
            ...
            <div class="modal-footer">
              <button type="button" class="btn btn-lg btn-secondary float-right" data-dismiss="modal"><i class="fas fa-times"></i> <?php echo lang('close'); ?></button>
              <button class="btn btn-primary btn-lg float-left" name="add_new_domain_submit" id="add_new_domain_submit" type="button"><i class="fa fa-save"></i> <?php echo lang('save');?></button>
            </div>
        </div>
    </div>
</div>