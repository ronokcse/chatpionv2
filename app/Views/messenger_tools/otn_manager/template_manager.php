    ...
    <table class="table table-bordered table-sm table-striped" id="mytable">
      <thead>
        <tr>
          ...
          <th><?php echo lang('OTN postback ID')?></th>
          <th><?php echo lang('Total OPTin subscribers')?></th>
          <th><?php echo lang('Message sent')?></th>
          <th><?php echo lang('Message not sent')?></th>
    ...
    <div class="modal-dialog" style="min-width: 95%;">
      <div class="modal-content">
        <div class="modal-header bbw">
          <h5 class="modal-title blue"><i class="fas fa-users"></i> <?php echo lang('OTN Subscribers');?></h5>
          <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
        </div>

          <div id="modalBody" class="modal-body">
            <input type="text" class="form-control" id="postback_id" autofocus placeholder="<?php echo lang('OTN PostBack ID'); ?>" aria-label="" aria-describedby="basic-addon2" style="max-width: 40%">
              ...
              <table class="table table-bordered table-sm table-striped" id="mytable2">
                <thead>
                  <tr>
                    ...
                    <th><?php echo lang('OTN PostBack'); ?></th> 
                    <th><?php echo lang('Subscriber ID'); ?></th>      
                    <th><?php echo lang('OPT-in Token'); ?></th>
                    <th><?php echo lang('OPT-in Time'); ?></th>
                  </tr>
                </thead>
              </table>
            </div>          
...
<?php 
  ...
  $drop_menu = '<a href="#" class="float-right btn btn-danger ml-2 get_otn_subscribers" page_table_id="'.$page_id.'" is_iframe="1"><i class="fa fa-users"></i> '.lang('OTN Subscribers').'</a>&nbsp;<a href="'.$new_otn_url_builder.'" target="_BLANK" class="float-right btn btn-primary" title="'.lang('Use Flow Builder').'"><i class="fab fa-stack-overflow"></i> '.lang('New Template').'</a>&nbsp;<a href="'.$new_otn_url_classic.'" class="btn btn-info float-right mr-2 iframed" title="'.lang('Use Classic Editor').'"><i class="fas fa-folder-plus"></i></a>';
    ...
    $(document).ready(function() {
      ...
      $(document).on('click','.delete_template',function(e){
        ...
        swal({
          ...
          text: '<?php echo lang('If you delete this template, all the token corresponding this template will also be deleted. Do you want to detete this template?'); ?>',
          icon: 'warning',
          buttons: true,
          dangerMode: true,
        })
          ...
          if (willDelete) 
          {
            ...
            $.ajax({
              ...
              success:function(response){ 
                ...
                if(response=='success')
                {
                  iziToast.success({title: '',message: '<?php echo lang('Template and all the corresponding token has been deleted successfully.'); ?>',position: 'bottomRight'});
                  table.draw();
                }
                else if(response=='no_match')
                {
                  iziToast.error({title: '',message: '<?php echo lang('No Template is found for this user with this ID.'); ?>',position: 'bottomRight'});
                }
                else
                {
    ...
    <div class="modal-dialog  modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title text-center"><i class="fa fa-trash"></i> <?php echo lang('Template Delete Confirmation'); ?></h5>
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
            </div>
