  ...
  <div class="modal-dialog" style="min-width: 60%;">
    <div class="modal-content">
      ...
      <div class="modal-header">
        <h5 class="modal-title text-center"><i class="fas fa-check-circle"></i> <?php echo lang('Enable Auto Like/Share');?></h5>
      ...
      <div class="modal-footer text-center">
        <button class="btn btn-lg btn-primary" id="autolikeshare_save_button"><i class="fa fa-save"></i> <?php echo lang('Save');?></button> 
      </div>
...
<div class="modal fade" id="pageresponse_auto_likeshare_edit_modal" data-backdrop="static" data-keyboard="false">>
  <div class="modal-dialog" style="min-width: 60%;">
    <div class="modal-content">
      ...
      <!-- Modal Header -->
      <div class="modal-header">
        <h5 class="modal-title text-center"><i class="fas fa-edit"></i> <?php echo lang('Edit Auto Like/Share');?></h5>
        <button type="button" class="close" data-dismiss="modal">&times;</button>
      </div>
      ...
      <!-- Modal footer -->
      <div class="modal-footer text-center">
        <button class="btn btn-lg btn-primary" id="autolikeshare_edit_button"><i class="fa fa-save"></i> <?php echo lang('Save');?></button> 
      </div>
...
<script>
  ...
  $(document).ready(function(){
    ...
    $(document).on('click','#autolikeshare_save_button',function(){      
      ...
      {
        swal('<?php echo lang('Warning'); ?>', '<?php echo lang('Please enable auto share or auto like.'); ?>', 'warning');
        return;
      }
      ...
      {
        swal('<?php echo lang('Warning'); ?>', '<?php echo lang('Please select page for auto sharing.'); ?>', 'warning');
        return;
      }

      if(auto_like_post=='1' && auto_like_this_post_by_pages.length==0)
      {
        swal('<?php echo lang('Warning'); ?>', '<?php echo lang('Please select page for auto liking.'); ?>', 'warning');
        return;
      }
    ...
    $(document).on('click','#autolikeshare_edit_button',function(){    
      ...
      $.ajax({
        ...
        success:function(response){
            ...
            if(response=="success")
            {
              swal('<?php echo lang('Success'); ?>', '<?php echo lang('campaign information has been saved successfully.');?>', 'success').then((value) => {
                $(".page_list_item.active").click();
