...
<div class="card" id="nodata">
  <div class="card-body">
    <div class="empty-state">
      ...
      <h2 class="mt-0"><?php echo lang('We could not find any page.');?></h2>
      <p class="lead"><?php echo lang('Please import account if you have not imported yet.')."<br>".lang('If you have already imported account then enable bot connection for one or more page to continue.') ?></p>
      <a href="<?php echo base_url('social_accounts'); ?>" class="btn btn-outline-primary mt-4"><i class="fas fa-arrow-circle-right"></i> <?php echo lang('Continue');?></a>
    </div>
  </div>
</div>
...
{ ?>
	...
	<?php if(file_exists(APPPATH.'show_comment_automation_message.txt') && session()->get('user_type') == 'Admin') : ?>
	<div class="row">
		<div class="col-12">
			<div class="alert alert-light alert-has-icon  alert-dismissible show fade" style="margin-bottom:30px"  id="hide_comment_automation_message" >
	          ...
	          <div class="alert-body">
	          	 <button class="close" data-dismiss="alert" data-toggle="tooltip" title="<?php echo lang('Close this mesage forever'); ?>">
	              	<span>×</span>
	             </button>
	            <div class="alert-title"><?php echo lang('Hello admin, please read me first'); ?></div>
	            <?php echo lang('Comment automation features will not work until your Facebook app is fully approved and is in live mode.'); ?>
	          </div>
	        </div>
		</div>
	</div>
...
<?php 
	...
	$Youdidntselectanyoption = lang('you have not select any option.');
	...
	$TypeAutoCampaignname = lang('You have not Type auto campaign name');
	$YouDidnotchosescheduleType = lang('You have not choose any schedule type');
	$YouDidnotchosescheduletime = lang('You have not select any schedule time');
	$YouDidnotchosescheduletimezone = lang('You have not select any time zone');
	$YoudidnotSelectPerodicTime = lang('You have not select any periodic time');
	$YoudidnotSelectCampaignStartTime = lang('You have not choose campaign start time');
	$YoudidnotSelectCampaignEndTime = lang('You have not choose campaign end time');
...
<script>
	...
	$(document).ready(function(){
		...
		$(document).on('click','#check_post_id',function(){
			...
			if(post_id=="")
			{
				swal('<?php echo lang('Warning'); ?>', '<?php echo lang('Please provide a post ID');?>', 'warning');
				return false;
			}
			...
			$.ajax({
			  ...
			  success:function(response){
				...
				if(response.error == 'yes')
					swal('<?php echo lang('Error'); ?>', response.error_msg, 'error');
				else
				{
		...
		$(document).on('click','.enable_auto_commnet',function(){
			...
			{
				alertify.alert('<?php echo lang('Alert')?>',Pleaseprovidepostid,function(){});
				return false;
			}
		...
		$(document).on('click','.save_button',function(){
			...
			if(typeof(use_template)==='undefined') 
			{
				...
				{
					swal('<?php echo lang('Warning'); ?>', Youdidntselectanyoption, 'warning');
					return false;
				}
				...
				if(reply_type == 'generic')
				{
					if(auto_campaign_name == ''){
						swal('<?php echo lang('Warning'); ?>', Youdidntprovideallinformation, 'warning');
						return false;
					}
				}
				else
			...
			else if(use_template == 'yes') 
			{
				...
				if(template_selection == '0') 
				{
					...
					swal('<?php echo lang('Warning'); ?>', Youdidntselectanytemplate, 'warning');
					return false;
				}
			}
		...
		$(document).on('click','#create_label_auto_reply',function(e){
		  e.preventDefault();
	  		...
	  		swal("<?php echo lang('Label Name'); ?>", {
		    	content: "input",
		    	button: {text: "<?php echo lang('Create'); ?>"},
		  	})
		  	.then((value) => {
		...
		$(document).on('click','.sync_commenter_info',function(){
		  ...
		  if(typeof(post_id) === 'undefined' || post_id == '')
		  {
			swal('<?php echo lang('Warning'); ?>', Pleaseprovidepostid, 'warning');
			return false;
		  }
		  ...
		  $.ajax({
			...
			success:function(response)
			{
				...
				if(response.status=='1')
				{
				  ...
				  swal({title:'<?php echo lang('Success'); ?>', content:span, icon:'success'}).then((value) => {
		                $(".page_list_item.active").click();
		                $("#manual_reply_by_post").modal('hide');
		                $("#manual_check_button_div").html('');
		              });
	...
	<div class="modal-dialog modal-lg" style="width:100%">
		<div class="modal-content">
			<div class="modal-header">
			  <h5 class="modal-title"><i class="fas fa-poll-h"></i> <?php echo lang('latest post for page') ?> - <span id="page_name_div"></span></h5>
...
<div class="modal fade" id="auto_reply_message_modal" data-backdrop="static" data-keyboard="false">
	<div class="modal-dialog modal-lg"  style="min-width: 70%;">
		<div class="modal-content">
			...
			<div class="modal-header">
			  <h5 class="modal-title" style="padding: 10px 20px 10px 20px;" ><?php echo lang('Please give the following information for post auto reply') ?></h5>
						...
						<div class="col-12 col-md-6">
						  <div class="form-group">
							<label class="custom-switch">
							  ...
							  <span class="custom-switch-description"><?php echo lang('Yes');?></span>
							</label>
						  </div>
						</div>
					</div>
...
Add label will only work once private reply is setup.  And you will need to sync subscribers later to update subscriber information. In this way the subscriber will not eligible for BOT subscriber until they reply back in messenger.').'"><i class="fa fa-info-circle"></i> </a>';
				      echo '<div class="row" style="padding-left: 20px; padding-right: 20px;">
				        <div class="col-3 col-md-3 hidden dropdown_con"> 
				            <div class="form-group">
				              ...
				              '.lang('Choose Labels').' '.$popover.'
				              </label>                                 
				              <label>
				              	<a class="blue float-right pointer" page_id_for_label="" id="create_label_auto_reply"><i class="fas fa-plus-circle"></i> '.lang('Create Label').'</a>
				              </label>
				            </div>       
				        </div>
					...
					<div id="new_template_section">
						...
						<div class="row" style="padding: 10px 20px 10px 20px; <?php if(!$commnet_hide_delete_addon) echo "display: none;"; ?> ">
							<div class="col-12" style="margin-bottom: 20px;">
								<div class="row">									
									<div class="col-12 col-md-6">
										<label><i class="fa fa-ban"></i> <?php echo lang('what do you want about offensive comments?') ?></label>
									</div>
									<div class="row">
									  <div class="col-12 col-md-6">
										<label class="custom-switch">
										  ...
										  <span class="custom-switch-indicator"></span>
										  <span class="custom-switch-description"><?php echo lang('hide'); ?></span>
										</label>
									  </div>
									  <div class="col-12 col-md-6">
										<label class="custom-switch">
										  ...
										  <span class="custom-switch-indicator"></span>
										  <span class="custom-switch-description"><?php echo lang('delete'); ?>
										</label>
									  </div>
									</div>
								</div>
								...
								<div class="row">								
									<div class="col-12 col-md-6" id="delete_offensive_comment_keyword_div">
										<div class="form-group" style="border: 1px dashed #e4e6fc; padding: 10px;">
											<label><i class="fa fa-tags"></i> <small><?php echo lang('write down the offensive keywords in comma separated') ?></small>
											</label>
											<textarea class="form-control message" name="delete_offensive_comment_keyword" id="delete_offensive_comment_keyword" placeholder="<?php echo lang('Type keywords here in comma separated (keyword1,keyword2)...Keep it blank for no actions') ?>" style="height:59px !important;"></textarea>
										</div>
									</div>
									...
									<div class="col-12 col-md-6">
										<div class="form-group clearfix" style="border: 1px dashed #e4e6fc; padding: 10px;">
											<label><small>
												<i class="fas fa-envelope"></i> <?php echo lang('Select a message template for private reply after deleting offensive comment') ?></small>
											</label>
											<div>                      
												<select class="form-group private_reply_postback" id="private_message_offensive_words" name="private_message_offensive_words">
													<option><?php echo lang('Please select a page first to see the message templates.'); ?></option>
												</select>

												<a href="" class="add_template float-left"><i class="fa fa-plus-circle"></i>     <?php echo lang('Add Message Template');?></a>
												<a href="" class="ref_template float-right"><i class="fa fa-refresh"></i> <?php echo lang('Refresh List');?></a>
						...
						<div class="row" style="padding: 10px 20px 10px 20px;">
							...
							<div class="col-12">
								<div class="row">									
									<div class="col-12 col-md-6"><label><i class="fa fa-sort-numeric-down"></i> <?php echo lang('do you want to send reply message to a user multiple times?') ?></label></div>
									<div class="col-12 col-md-6">
									  <div class="form-group">
										<label class="custom-switch">
							...
							<div class="col-12">
								<div class="row">									
									<div class="col-12 col-md-6">
										<label><i class="fa fa-comment-dots"></i> <?php echo lang('do you want to enable comment reply?') ?></label>
									</div>
									<div class="col-12 col-md-6">
									  <div class="form-group">
										<label class="custom-switch">
							...
							<div class="col-12">
								<div class="row">									
									<div class="col-12 col-md-6">
										<label><i class="fa fa-comment"></i> <?php echo lang('do you want to like on comment by page?') ?></label>
									</div>
									<div class="col-12 col-md-6">
									  <div class="form-group">
										<label class="custom-switch">
							...
							<div class="col-12" <?php if(!$commnet_hide_delete_addon) echo "style='display: none;'"; ?> >
								<div class="row">									
									<div class="col-12 col-md-6">
										<label><i class="fa fa-eye-slash"></i>  <?php echo lang('do you want to hide comments after comment reply?') ?></label>
									</div>
									<div class="col-12 col-md-6">
										<div class="form-group">
										  <label class="custom-switch">
							...
							<div class="col-12">
								...
								<?php if(ai_reply_exist()) : ?>
								  <div class="custom-control custom-radio">
									...
									<label class="custom-control-label" for="ai_reply"><?php echo lang('automated reply by AI') ?></label>
								  </div>
								<?php endif; ?>

							  <div class="custom-control custom-radio">
								<input type="radio" name="message_type" value="generic" id="generic" class="custom-control-input radio_button">
								<label class="custom-control-label" for="generic"><?php echo lang('generic message for all') ?></label>
							  </div>
							  <div class="custom-control custom-radio">
								<input type="radio" name="message_type" value="filter" id="filter" class="custom-control-input radio_button">
								<label class="custom-control-label" for="filter"><?php echo lang('send message by filtering word/sentence') ?></label>
							  </div>
							</div>
							...
							<div class="col-12" style="margin-top: 15px;">
								<div class="form-group">
									<label>
										<i class="fas fa-monument"></i> <?php echo lang('auto reply campaign name') ?> <span class="red">*</span>
									</label>
									<input class="form-control" type="text" name="auto_campaign_name" id="auto_campaign_name" placeholder="<?php echo lang('write your auto reply campaign name here') ?>">
								</div>
							</div>
							...
							<div class="col-12" id="ai_message_div" style="display: none;">
								<div class="form-group clearfix" style="border: 1px dashed #e4e6fc; padding: 20px;">
									<label>
										<i class="fa fa-robot"></i> <?php echo lang('AI training data') ?> <span class="red">*</span>
									</label>
									<textarea class="form-control" name="ai_training_data" id="ai_training_data" placeholder="<?php echo lang('type your message here...') ?>" style="height:170px !important;"></textarea>
								</div>

								<label>
								  <i class="fas fa-envelope"></i> <?php echo lang('Select a message template for private reply') ?> [<?php echo lang('Maximum two reply message is supported.'); ?>]
								</label>
								<div>                      
							...
							<div class="col-12" id="generic_message_div" style="display: none;">
								<div class="form-group clearfix" style="border: 1px dashed #e4e6fc; padding: 20px;">
									<label>
										<i class="fa fa-envelope"></i> <?php echo lang('Message for comment reply') ?> <span class="red">*</span>
										<a href="#" data-placement="bottom"  data-toggle="popover" data-trigger="focus" title="<?php echo lang('message') ?>" data-content="<?php echo lang('write your message which you want to send. You can customize the message by individual commenter name.'); ?>  Spintax example : {Hello|Howdy|Hola} to you, {Mr.|Mrs.|Ms.} {{Jason|Malina|Sara}|Williams|Davis}"><i class='fa fa-info-circle'></i> </a>
									</label>
									<?php if($comment_tag_machine_addon) {?>
									<span class='float-right'> 
										<a title="<?php echo lang('You can tag user in your comment reply. Facebook will notify them about mention whenever you tag.') ?>" data-toggle="tooltip" data-placement="top" class='btn-sm lead_tag_name button-outline'><i class='fa fa-tags'></i>  <?php echo lang('tag user') ?></a>
									</span>
									<?php } ?>
									<span class='float-right'> 
									  <a title="<?php echo lang('You can include #LEAD_USER_LAST_NAME# variable inside your message. The variable will be replaced by real names when we will send it.') ?>" data-toggle="tooltip" data-placement="top" class='btn-sm lead_last_name button-outline'><i class='fa fa-user'></i> <?php echo lang('last name') ?></a>
									</span>
									<span class='float-right'> 
									  <a title="<?php echo lang('You can include #LEAD_USER_FIRST_NAME# variable inside your message. The variable will be replaced by real names when we will send it.') ?>" data-toggle="tooltip" data-placement="top" class='btn-sm lead_first_name button-outline'><i class='fa fa-user'></i> <?php echo lang('first name') ?></a>
									</span> 
									
									<div class="clearfix"></div>						
									<textarea class="form-control message" name="generic_message" id="generic_message" placeholder="<?php echo lang('type your message here...') ?>" style="height:170px !important;"></textarea>

									<!-- comment hide and delete section -->
									<br/>
									<div class="row clearfix" <?php if(!$commnet_hide_delete_addon) echo "style='display: none;'"; ?> >
										<div class="col-12 col-md-6">
											<label class="control-label" ><i class="fa fa-image"></i> <?php echo lang('image for comment reply') ?></label>									
											<div class="form-group">      
												<div id="generic_comment_image"><?php echo lang('upload') ?></div>	     
											</div>
											<div id="generic_image_preview_id"></div>
											<span class="red" id="generic_image_for_comment_reply_error"></span>
											<input type="text" name="generic_image_for_comment_reply" class="form-control" id="generic_image_for_comment_reply" placeholder="<?php echo lang('put your image url here or click the above upload button') ?>" style="margin-top: -14px;" />
										</div>

										<div class="col-12 col-md-6">
											<label class="control-label" ><i class="fa fa-youtube"></i>  <?php echo lang('video for comment reply') ?> [mp4 <?php echo lang('Prefered');?>]
												<a href="#" data-placement="bottom" data-toggle="popover" data-trigger="focus" title="<?php echo lang('video upload') ?>" data-content="<?php echo lang('image and video will not work together. Please choose either image or video.') ?> [mp4,wmv,flv]"><i class='fa fa-info-circle'></i></a>
											</label>
											<div class="form-group">      
												<div id="generic_video_upload"><?php echo lang('upload') ?></div>	     
											</div>
											<div id="generic_video_preview_id"></div>
											<span class="red" id="generic_video_comment_reply_error"></span>
											<input type="hidden" name="generic_video_comment_reply" class="form-control" id="generic_video_comment_reply" placeholder="<?php echo lang('Put your image url here or click upload') ?>"  />
										</div>
									</div>
									<br/><br/>
									<!-- comment hide and delete section -->
							...
							<div class="col-12" id="filter_message_div">
									<div class="row">
									  <div class="col-12 col-md-6">
										<label class="custom-switch">
										  ...
										  <span class="custom-switch-indicator"></span>
										  <span class="custom-switch-description"><?php echo lang('Reply if the filter word exactly matches.'); ?></span>
										</label>
									  </div>
									  <div class="col-12 col-md-6">
										<label class="custom-switch">
										  ...
										  <span class="custom-switch-indicator"></span>
										  <span class="custom-switch-description"><?php echo lang('Reply if any matches occurs with filter word.'); ?>
										</label>
									  </div>
									</div><br/>
								<?php for ($i=1; $i <= 20 ; $i++) : ?>
										<div class="form-group clearfix" id="filter_div_<?php echo $i; ?>" style="border: 1px dashed #e4e6fc; padding: 20px; margin-bottom: 50px;">
											<label>
												<i class="fa fa-tag"></i> <?php echo lang('filter word/sentence') ?> <span class="red">*</span>
												<a href="#" data-placement="bottom"  data-toggle="popover" data-trigger="focus" title="<?php echo lang('message') ?>" data-content="<?php echo lang('Write the word or sentence for which you want to filter comment. For multiple filter keyword write comma separated. Example -   why, wanto to know, when') ?>"><i class='fa fa-info-circle'></i> </a>
											</label>
											<input class="form-control filter_word" type="text" name="filter_word_<?php echo $i; ?>" id="filter_word_<?php echo $i; ?>" placeholder="<?php echo lang('write your filter word here') ?>">
											
											
											<!-- new feature comment reply section -->
											<br/>
											<label>
												<i class="fa fa-envelope"></i> <?php echo lang('msg for comment reply') ?><span class="red">*</span>
												<a href="#" data-placement="bottom"  data-toggle="popover" data-trigger="focus" title="<?php echo lang('message') ?>" data-content="<?php echo lang('write your message which you want to send based on filter words. You can customize the message by individual commenter name.'); ?>  Spintax example : {Hello|Howdy|Hola} to you, {Mr.|Mrs.|Ms.} {{Jason|Malina|Sara}|Williams|Davis}"><i class='fa fa-info-circle'></i> </a>
											</label>
											<?php if($comment_tag_machine_addon) {?>
											<span class='float-right'> 
												<a title="<?php echo lang('You can tag user in your comment reply. Facebook will notify them about mention whenever you tag.') ?>" data-toggle="tooltip" data-placement="top" class='btn-sm lead_tag_name button-outline'><i class='fa fa-tags'></i>  <?php echo lang('tag user') ?></a>
											...
											<div class="clearfix"></div>
											<textarea class="form-control message" name="comment_reply_msg_<?php echo $i; ?>" id="comment_reply_msg_<?php echo $i; ?>"  placeholder="<?php echo lang('type your message here...') ?>" style="height:170px !important;"></textarea>
											...
											<div class="clearfix" <?php if(!$commnet_hide_delete_addon) echo "style='display: none;'"; ?> >
												<div class="row">													
													<div class="col-12 col-md-6">
														<label class="control-label" ><i class="fa fa-image"></i> <?php echo lang('image for comment reply') ?>
														</label>									
														<div class="form-group">      
															<div id="filter_image_upload_<?php echo $i; ?>"><?php echo lang('upload') ?></div>	     
														</div>
														<div id="generic_image_preview_id_<?php echo $i; ?>"></div>
														<span class="red" id="generic_image_for_comment_reply_error_<?php echo $i; ?>"></span>
														<input type="text" name="filter_image_upload_reply_<?php echo $i; ?>" class="form-control" id="filter_image_upload_reply_<?php echo $i; ?>" placeholder="<?php echo lang('Put your image url here or click the above upload button') ?>" style="margin-top: -14px;" />
													</div>
													...
													<div class="col-12 col-md-6">
														...
														<div class="form-group">      
															<div id="filter_video_upload_<?php echo $i; ?>"><?php echo lang('upload') ?></div>	     
														</div>
														<div id="generic_video_preview_id_<?php echo $i; ?>"></div>
														<span class="red" id="edit_generic_video_comment_reply_error_<?php echo $i; ?>"></span>
														<input type="hidden" name="filter_video_upload_reply_<?php echo $i; ?>" class="form-control" id="filter_video_upload_reply_<?php echo $i; ?>" placeholder="<?php echo lang('Put your image url here or click upload') ?>"  />
													</div>
												</div>
											</div>
											<!-- comment hide and delete section -->
								...
								<div class="clearfix">
									...
									<button type="button" class="btn btn-sm btn-outline-primary float-right" id="add_more_button"><i class="fa fa-plus"></i> <?php echo lang('add more filtering') ?></button>
								</div>
								...
								<div class="form-group clearfix" id="nofilter_word_found_div" style="margin-top: 10px; border: 1px dashed #e4e6fc; padding: 20px;">
									...
									<div class="clearfix"></div>
									<textarea class="form-control message" name="nofilter_word_found_text" id="nofilter_word_found_text"  placeholder="<?php echo lang('type your message here...') ?>" style="height:170px !important;"></textarea>
									...
									<div class="clearfix" <?php if(!$commnet_hide_delete_addon) echo "style='display: none;'"; ?> >
										<div class="row">											
											<div class="col-12 col-md-6">
												...
												<div class="form-group">      
													<div id="nofilter_image_upload"><?php echo lang('upload') ?></div>	     
												</div>
												<div id="nofilter_generic_image_preview_id"></div>
												<span class="red" id="nofilter_image_upload_reply_error"></span>
												<input type="text" name="nofilter_image_upload_reply" class="form-control" id="nofilter_image_upload_reply" placeholder="<?php echo lang('put your image url here or click the above upload button') ?>" style="margin-top: -14px;" />
											</div>
											...
											<div class="col-12 col-md-6">
												...
												<div class="form-group">      
													<div id="nofilter_video_upload"><?php echo lang('upload') ?></div>	     
												</div>
												<div id="nofilter_video_preview_id"></div>
												<span class="red" id="nofilter_video_upload_reply_error"></span>
												<input type="hidden" name="nofilter_video_upload_reply" class="form-control" id="nofilter_video_upload_reply" placeholder="<?php echo lang('put your image url here or click upload') ?>"  />
											</div>
										</div>
									</div>
									<br/><br/>
									...
									<label>
									  <i class="fas fa-envelope"></i> <?php echo lang('Select a message template for private reply if no matching found') ?> [<?php echo lang('Maximum two reply message is supported.'); ?>]
									</label>
									<div>                      
			...
			<div class="modal-footer" style="padding-left: 45px; padding-right: 45px; ">
				<div class="row">
					<div class="col-6">
						<button class="btn btn-lg btn-info save_button float-left" id="save_and_create" button_name="submit_create_button"><i class='fa fa-save'></i> <?php echo lang('submit & save as template') ?></button>
					</div>  
					<div class="col-6">
						<button class="btn btn-lg btn-primary save_button float-right" id="save_only" button_name="only_submit"><i class='fa fa-save'></i> <?php echo lang('submit') ?></button>
					</div>
				</div>
			</div>
...
Add label will only work once private reply is setup.  And you will need to sync subscribers later to update subscriber information. In this way the subscriber will not eligible for BOT subscriber until they reply back in messenger.').'"><i class="fa fa-info-circle"></i> </a>';
			      echo '<div class="row" style="padding-left: 20px; padding-right: 20px;">
			        <div class="col-3 col-md-3 hidden dropdown_con"> 
			            <div class="form-group">
			              ...
			              <label>
			              	<a class="blue float-right pointer" page_id_for_label="" id="create_label_edit_auto_reply"><i class="fas fa-plus-circle"></i> '.lang('Create Label').'</a>
			              </label>                              
			            </div>       
			        </div>
			        <div class="col-9 col-md-9 hidden dropdown_con"> 
				...
				<div class="row" style="padding: 20px;<?php if(!$commnet_hide_delete_addon) echo "display: none;"; ?> ">
					...
					<div class="col-12 col-md-6" id="edit_delete_offensive_comment_keyword_div">
						<div class="form-group clearfix" style="border: 1px dashed #e4e6fc; padding: 10px;">
							...
							<textarea class="form-control message" name="edit_delete_offensive_comment_keyword" id="edit_delete_offensive_comment_keyword" placeholder="<?php echo lang('Type keywords here in comma separated (keyword1,keyword2)...Keep it blank for no actions') ?>" style="height:59px !important;"></textarea>
						</div>
					</div>
				...
				<div class="row" style="padding: 10px 20px 10px 20px;">
					...
					<div class="col-12">
						<?php if(ai_reply_exist()) : ?>
						  <div class="custom-control custom-radio">
							...
							<label class="custom-control-label" for="edit_ai_reply"><?php echo lang('automated reply by AI') ?></label>
						  </div>
						<?php endif; ?>
					  <div class="custom-control custom-radio">
					  	<input type="radio" name="edit_message_type" value="generic" id="edit_generic" class="custom-control-input radio_button">
					  	<label class="custom-control-label" for="edit_generic"><?php echo lang('generic message for all') ?></label>
					  </div>
					  <div class="custom-control custom-radio">
					  	<input type="radio" name="edit_message_type" value="filter" id="edit_filter" class="custom-control-input radio_button">
					  	<label class="custom-control-label" for="edit_filter"><?php echo lang('send message by filtering word/sentence') ?></label>
					  </div>
					</div>
					...
					<div class="col-12" style="margin-top: 15px;">
						<div class="form-group">
							<label>
								<i class="fa fa-monument"></i> <?php echo lang('auto reply campaign name') ?> <span class="red">*</span>
							</label>
							<input class="form-control" type="text" name="edit_auto_campaign_name" id="edit_auto_campaign_name" placeholder="<?php echo lang('write your auto reply campaign name here') ?>">
						</div>
					</div>
					...
					<div class="col-12" id="edit_ai_message_div" style="display: none;">
						<div class="form-group clearfix" style="border: 1px dashed #e4e6fc; padding: 20px;">
							...
							<textarea class="form-control" name="edit_ai_training_data" id="edit_ai_training_data" placeholder="<?php echo lang('type your message here...') ?>" style="height:170px !important;"></textarea>
						</div>
					...
					<div class="col-12" id="edit_generic_message_div" style="display: none;">
						<div class="form-group clearfix" style="border: 1px dashed #e4e6fc; padding: 20px;">
							...
							<div class="clearfix"></div>
							<textarea class="form-control message" name="edit_generic_message" id="edit_generic_message" placeholder="<?php echo lang('type your message here...') ?>" style="height:170px !important;"></textarea>
							...
							<div class="clearfix" <?php if(!$commnet_hide_delete_addon) echo "style='display: none;'"; ?> >
								<div class="row">									
									<div class="col-12 col-md-6">
										...
										<div class="form-group">      
											<div id="edit_generic_comment_image"><?php echo lang('upload') ?></div>	     
										</div>
										<div id="edit_generic_image_preview_id"></div>
										<span class="red" id="generic_image_for_comment_reply_error"></span>
										<input type="text" name="edit_generic_image_for_comment_reply" class="form-control" id="edit_generic_image_for_comment_reply" placeholder="<?php echo lang('put your image url here or click the above upload button') ?>" style="margin-top: -14px;" />
									...
									<div class="col-12 col-md-6">
										...
										<div class="form-group">      
											<div id="edit_generic_video_upload"><?php echo lang('upload') ?></div>	     
										</div>
										<div id="edit_generic_video_preview_id"></div>
										<span class="red" id="edit_generic_video_comment_reply_error"></span>
										<input type="hidden" name="edit_generic_video_comment_reply" class="form-control" id="edit_generic_video_comment_reply" placeholder="<?php echo lang('put your image url here or click upload') ?>" />
										...
										<div class="overlay_wrapper">
											...
											<?php echo lang('your browser does not support the video tag.') ?>
											</video>
										</div>
									</div>
								</div>
					...
					<?php for($i=1;$i<=20;$i++) :?>
						<div class="form-group clearfix" id="edit_filter_div_<?php echo $i; ?>" style="border: 1px dashed #e4e6fc; padding: 20px; margin-bottom: 50px;">
							<label>
								<i class="fa fa-tag"></i> <?php echo lang('filter word/sentence') ?> <span class="red">*</span>
								<a href="#" data-placement="bottom"  data-toggle="popover" data-trigger="focus" title="<?php echo lang('message') ?>" data-content="<?php echo lang('Write the word or sentence for which you want to filter comment. For multiple filter keyword write comma separated. Example -   why, want to know, when') ?>"><i class='fa fa-info-circle'></i> </a>
							</label>
							<input class="form-control filter_word" type="text" name="edit_filter_word_<?php echo $i; ?>" id="edit_filter_word_<?php echo $i; ?>" placeholder="<?php echo lang('write your filter word here') ?>">
							...
							<div class="clearfix"></div>
							<textarea class="form-control message" name="edit_comment_reply_msg_<?php echo $i; ?>" id="edit_comment_reply_msg_<?php echo $i; ?>"  placeholder="<?php echo lang('type your message here...') ?>" style="height:170px !important;"></textarea>
							...
							<div class="clearfix" <?php if(!$commnet_hide_delete_addon) echo "style='display: none;'"; ?> >
								<div class="row">									
									<div class="col-12 col-md-6">
										...
										<div class="form-group">      
											<div id="edit_filter_image_upload_<?php echo $i; ?>"><?php echo lang('upload') ?></div>	     
										</div>
										...
										<input type="text" name="edit_filter_image_upload_reply_<?php echo $i; ?>" class="form-control" id="edit_filter_image_upload_reply_<?php echo $i; ?>" placeholder="<?php echo lang('put your image url here or click the above upload button') ?>" style="margin-top: -14px;" />
									...
									<div class="col-12 col-md-6">
										...
										<div class="form-group">      
											<div id="edit_filter_video_upload_<?php echo $i; ?>"><?php echo lang('upload') ?></div>	     
										</div>
										<div id="edit_generic_video_preview_id_<?php echo $i; ?>"></div>
										<span class="red" id="edit_generic_video_comment_reply_error_<?php echo $i; ?>"></span>
										<input type="hidden" name="edit_filter_video_upload_reply_<?php echo $i; ?>" class="form-control" id="edit_filter_video_upload_reply_<?php echo $i; ?>" placeholder="<?php echo lang('put your image url here or click upload') ?>"  />
					...
					<?php endfor; ?>
						...
						<div class="clearfix">
							...
							<button type="button" class="btn btn-sm btn-outline-primary float-right" id="edit_add_more_button"><i class="fa fa-plus"></i> <?php echo lang('add more filtering') ?></button>
						</div>
						...
						<div class="form-group clearfix" id="edit_nofilter_word_found_div" style="margin-top: 10px; border: 1px dashed #e4e6fc; padding: 20px;">
							...
							<div class="clearfix"></div>
							<textarea class="form-control message" name="edit_nofilter_word_found_text" id="edit_nofilter_word_found_text"  placeholder="<?php echo lang('type your message here...') ?>" style="height:170px !important;"></textarea>
							...
							<div class="clearfix" <?php if(!$commnet_hide_delete_addon) echo "style='display: none;'"; ?> >
								<div class="row">									
									<div class="col-12 col-md-6">
										...
										<div class="form-group">      
											<div id="edit_nofilter_image_upload"><?php echo lang('upload') ?></div>	     
										</div>
										<div id="edit_nofilter_generic_image_preview_id"></div>
										<span class="red" id="edit_nofilter_image_upload_reply_error"></span>
										<input type="text" name="edit_nofilter_image_upload_reply" class="form-control" id="edit_nofilter_image_upload_reply" placeholder="<?php echo lang('put your image url here or click the above upload button') ?>" style="margin-top: -14px;" />
									...
									<div class="col-12 col-md-6">
										...
										<div class="form-group">      
											<div id="edit_nofilter_video_upload"><?php echo lang('upload') ?></div>	     
										</div>
										<div id="edit_nofilter_video_preview_id"></div>
										<span class="red" id="edit_nofilter_video_upload_reply_error"></span>
										<input type="hidden" name="edit_nofilter_video_upload_reply" class="form-control" id="edit_nofilter_video_upload_reply" placeholder="<?php echo lang('put your image url here or click upload') ?>" />
...
<div class="modal fade" id="manual_reply_by_post" data-backdrop="static" data-keyboard="false">
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			<div class="modal-header">
			  <h5 class="modal-title"><?php echo lang('Enable reply by Post ID') ?> (<span id="manual_page_name"></span>)</h5>
			  <button type="button" class="close" data-dismiss="modal" aria-label="Close">
				<span aria-hidden="true">×</span>
			  </button>
			</div>
					...
					<div class="col-12">
						<form>
							...
							<div class="input-group">
		                        <div class="input-group-prepend">
		                          <div class="input-group-text">
		                            <?php echo lang('post id') ?>
		                          </div>
		                        </div>
		                        <input type="text" class="form-control" id="manual_post_id" placeholder="<?php echo lang('please give a post id') ?>">
		                    </div>
						 ...
						 <div class="form-group text-center">
							<button type="button" class="btn btn-info" id="check_post_id"><i class="fa fa-check"></i> <?php echo lang('check existance') ?></button>
						 </div>
	...
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h5 class="modal-title"><?php echo lang('please provide a post id of page') ?> (<span id="manual_edit_page_name"></span>)</h5>
			</div>
			<div class="modal-body ">
				<div class="row">
					<div class="col-12" id="waiting_div"></div>
					<div class="col-12 col-md-8 col-md-offset-2">
						<form>
							<div class="form-group">
							  ...
							  <input type="text" class="form-control" id="manual_edit_post_id" placeholder="<?php echo lang('please give a post id') ?>" value="">
							  <input type="hidden" id="manual_edit_table_id">
							</div>
							<div class="text-center" id="manual_edit_error"></div>                           
						</form>
						<div class="form-group text-center" style="margin-top: 15px;">
						   <button type="button" class="btn btn-outline-warning edit_reply_info" id="manual_edit_auto_reply"><i class="fa fa-edit"></i> <?php echo lang('edit auto reply') ?></button>
						</div>
  ...
  <div class="modal-dialog modal-lg">
	<div class="modal-content">
	  <div class="modal-header clearfix">
		...
		<h5 class="modal-title"><i class="fa fa-file-video-o"></i> <?php echo lang('filemanager Library') ?></h5>
	  </div>
	  <div class="modal-body">
...
<div class="modal fade" id="instant_comment_modal" data-backdrop="static" data-keyboard="false">
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
			  <h5 class="modal-title"><i class="fas fa-comments"></i> <?php echo lang('Instant Comment') ?></h5>
			  <button type="button" class="close" data-dismiss="modal" aria-label="Close">
				<span aria-hidden="true">&times;</span>
			  </button>
			</div>
			...
			<div class="modal-body">     
				...
				<div class="row">
					<div class="col-12">
						<div class="form-group">
							<label><i class="fas fa-keyboard"></i> <?php echo lang('Please provide a message as comment') ?></label>
							<textarea class="form-control" name="instant_comment_message" id="instant_comment_message" placeholder="<?php echo lang('Type your comment here.') ?>"></textarea>
						</div>
					</div>
					<div class="col-12">
						<button class="btn btn-primary submit_instant_comment"><i class="fas fa-paper-plane"></i> <?php echo lang('Create Comment'); ?></button>
					</div>
				</div>
			</div>
		</div>
...
<script>
	$("document").ready(function(){
		...
		$(document).on('click','.submit_instant_comment',function(){
			...
			if(message == '')
			{
				swal('<?php echo lang('Warning'); ?>', '<?php echo lang('Please provide your comment first.'); ?>', 'warning');
				return false;
			}
		...
		var session_value = "<?php echo session()->get('get_page_details_page_table_id'); ?>";
		if(session_value==''){
...
<script type="text/javascript">
	$("document").ready(function(){		
		...
		$(document).on('click','.add_template',function(e){
		  ...
		  if(page_id=="")
		  {
		    swal('<?php echo lang('Error'); ?>', "<?php echo lang('Please select a page first')?>", 'error');
		    return false;
		  }
...
<script type="text/javascript">
			...
			if (willDelete) 
			{
				...
				$.ajax({
					...
					success:function(response){ 
						...
						{
							var span = document.createElement("span");
							span.innerHTML = '<?php echo lang('You do not have permission to access this module.'); ?>';
							swal({ title:'<?php echo lang('Error!'); ?>', content:span,icon:'error'});
						}
					}
				});
			...
			if (willDelete) 
			{
				...
				$.ajax({
					...
					success:function(response){ 
						if(response == 'success')
						{
							iziToast.success({title: '',message: '<?php echo lang('Campaign has been started successfully.'); ?>',position: 'bottomRight'});
							$(".page_list_item.active").click();
						}
						if(response == 'error')
						{
	...
	$(document).on('click','.delete_single_comment',function(e){
		...
		var mes='<?php echo lang('Do you really want to delete this comment?');?>';  
		$(this).addClass('btn-progress');
		swal({
			context: this,
			title: "<?php echo lang('Are you sure?');?>",
	...
	$(document).on('click','.block_single_commenter',function(e){
		...
		var mes='<?php echo lang('Do you really want to block this person from commenting on your post?');?>';  
		$(this).addClass('btn-progress');
		swal({
			context: this,
			title: "<?php echo lang('Are you sure?');?>",
	...
	$(document).on('click','.send_single_reply',function(e){
		...
		if(message == '')
		{
			swal('<?php echo lang('Warning'); ?>', '<?php echo lang('Please provide your reply message');?>', 'warning');
			return false;
		}
		$(this).addClass('btn-progress');
...
<div class="modal fade" id="view_report_modal" data-backdrop="static" data-keyboard="false">
  <div class="modal-dialog modal-mega">
      <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title"><i class="far fa-list-alt"></i> <?php echo lang('Report of Auto Reply');?>
            	<small>
            	(
            	<?php 
    ...
    <div class="modal-dialog modal-mega">
        <div class="modal-content">
            ...
            <div class="modal-body data-card">
                <div class="row">
          			<div class="col-12 col-md-9">
      			  		<input type="text" id="autocomment_searching" name="autocomment_searching" class="form-control width_200px" placeholder="<?php echo lang('Search...'); ?>">                                          
          			</div>
                    <div class="col-12">
                      <div class="table-responsive2">
