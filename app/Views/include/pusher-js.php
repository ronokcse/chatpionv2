...
<?php 
  // CI4: Configs are auto-loaded, no need for config->load()
  $media_text = '';
  $mediaType = isset($using_media_type) ? $using_media_type : 'fb';
  if($mediaType == 'fb'){
    $media_text = 'Facebook';
  }
  else{
  ...
  $get_real_user_id = session()->get('real_user_id');
?>
<script>
  var get_real_user_id = "<?php echo $get_real_user_id;?>";
  // Enable pusher logging - don't include this in production
  Pusher.logToConsole = false;
  var pusher = new Pusher('<?php echo config('MyConfig')->pusher_app_key;?>', {
    cluster: '<?php echo config('MyConfig')->pusher_cluster ?? '';?>'
  });
  var isNotitificationEnabled = '<?php echo $browser_notification_enabled ?? '0';?>';
  var timezone = '<?php echo !empty(config('MyConfig')->time_zone) ? config('MyConfig')->time_zone : 'Europe/Dublin';?>';
  ...
  var whatsAppPushChannel = pusher.subscribe('livechat_channel');
    whatsAppPushChannel.bind('livechat_event', function(responseData) {
      ...
      if(messageType != 'archived'){
          if(responseData[5]=='user' && responseData[0] == get_real_user_id) {
            ...
            if(isNotitificationEnabled=='1'){
              Notification.requestPermission().then(perm=>{
                if(perm == 'granted'){
                  ...
                  const notification = new Notification(subscriberName+" <?php echo lang('sent you a message on') .' '.$media_text;?>",{
  ...
  assignedAgentPushChannel.bind('livechat_assigned_agent_event', function(responseData) {
    ...
    if(assignedAgentId == get_real_user_id) {
    ...
    if(state=='0'){// enable notification
        ...
        if(Notification.permission == "granted") notification_action('1');
          ...
          Notification.requestPermission().then((perm) => {
            ...
            else iziToast.warning({title: 'Warning',message:'<?php echo lang('Please allow browser notification permission to get inbox notification.');?>',position: 'bottomRight'});
          });
        }
        else iziToast.warning({title: 'Warning',message:'<?php echo lang('Notification permission is blocked by your browser. Please allow notification permission for this site manually to get inbox notification.');?>',position: 'bottomRight'});
      });
    }
  ...
  function notification_action(action)
  {
    $.ajax({
        ...
        beforeSend: function (xhr) {
          xhr.setRequestHeader('X-CSRF-TOKEN', '<?php echo csrf_hash();?>');
        },
