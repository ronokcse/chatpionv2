...
<section class="section">
	...
	$options[''] = lang('Store')." : ".$xdata['store_name'];
	foreach ($ecommerce_cart_pickup_points as $key => $value)
	{
		$options[$value['id']] =  lang('Delievry point')." - ".$value['point_name']." : ".$value['point_details'];
	}
				...
				<div class="row">
					<div class="col-12">
						<div class="form-group">
							<label><?php echo lang('Select store or delivery point');?></label>
					...
					<div class="col-12 col-md-6">						
						<div class="card" style="border:1px solid #dee2e6">
							<div class="card-header text-center">
								<h6 class="full_width"><?php echo lang('Messenger QR Code'); ?>
								</h6>
							</div>				
							<div class="card-body" style="min-height: 680px">
								<div class="form-group">
			                      <div class="input-group">
			                        <div class="input-group-prepend"><span class="input-group-text"><?php echo lang('Foreground'); ?></span></div>
			                        <input type="color" value="<?php echo $msg_fore_color; ?>" class="form-control" name="msg_fore_color" id="msg_fore_color">
			                         <div class="input-group-prepend"><span class="input-group-text"><?php echo lang('Background'); ?></span></div>
			                        <input type="color" value="<?php echo $msg_back_color; ?>" class="form-control" name="msg_back_color" id="msg_back_color">
			                      </div>
			                    </div>
								<div class="text-center">
									<a href="<?php echo base_url('ecommerce/download_qr/'.$qr_img['messenger_qr']);?>" class="btn btn-sm mb-2 btn-outline-primary"><i class="fas fa-download"></i> <?php echo lang('Download'); ?></a><br>
								...
								<br>
								<p class="mt-3 mb-3">
									<b><?php echo lang('Welcome Message'); ?></b>									
									<a id="variables" class="float-right text-warning pointer"><i class="fas fa-circle"></i> <?php echo  lang('Variables'); ?></a>
								</p>
		                 		<div class="reminder_block">
		                 			<span class="block4">
		                 				<textarea style="height: 50px" data-toggle="tooltip" title="<?php echo lang('After scanning QR code this message will be displayed inside Messenger. Click to edit text.'); ?>" name="msg_text" id="msg_text"><?php echo isset($qr_code['msg_text']) ? $qr_code['msg_text'] : "Hi {{first_name}}, welcome to our store."; ?></textarea>	                         	
		                 				<p>
		                 				<input data-toggle="tooltip" title="<?php echo lang('Clicking this button will load your store inside Messenger. Click to edit button name.'); ?>" value="<?php echo isset($qr_code['msg_btn']) ? $qr_code['msg_btn'] : 'START'; ?>" class="btn btn-block bg-white" name="msg_btn" id="msg_btn"/>
		                 				</p>
		                 			</span>
		                 		</div>
							</div>
					...
					<div class="col-12 <?php if($qr_img['messenger_qr']!="") echo 'col-md-6';?>">				
						<div class="card" style="border:1px solid #dee2e6">
							...
							<div class="card-body" style="min-height: 680px">
								...
								<div class="text-center">
									<a href="<?php echo base_url('ecommerce/download_qr/'.$qr_img['public_qr']);?>" class="btn btn-sm mb-2 btn-outline-primary"><i class="fas fa-download"></i> <?php echo lang('Download'); ?></a><br>
...
$(document).ready(function($) { 
  $(document).on('click','#variables',function(e){
    ...
    swal({ title:'<?php echo lang('Variables'); ?>', content:span,icon:'info'});     
  });
    ...
    $.ajax({
      ...
      {  
        ...
        { 
          ...
          else swal('<?php echo lang('Settings Updated'); ?>', response.message, 'success').then((value) => { parent.window.location.assign('<?php echo base_url("ecommerce/store_list") ?>');});
        }
