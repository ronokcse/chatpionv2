      ...
      <div class="hero p-2" style="border-radius: 0 0 3px 3px;">
        <div class="hero-inner">
          <ul class="nav nav-tabs" id="myTab2" role="tablist">
            ...
            <li class="nav-item">
              <a class="nav-link active show" id="details-tab2" data-toggle="tab" href="#details" role="tab" aria-controls="details" aria-selected="false"><?php echo lang('Options'); ?></a>
            </li>
            <?php endif; ?>
            <?php if(!empty($product_data['product_description']) || !empty($product_data['product_video_id'])): ?>
            <li class="nav-item">
              <a class="nav-link <?php echo (!$have_attributes) ? 'active show' : '';?>" id="description-tab2" data-toggle="tab" href="#description" role="tab" aria-controls="description" aria-selected="false"><?php echo lang('Details'); ?></a>
            </li>
            <?php endif; ?>   
              ...
              echo '
                ...
                <div class="card-body pb-2">
                  <div class="empty-state p-0">
                    <h6 class="mt-0">'.lang('We could not find any review.').'</h6>
                  </div>
                </div>
              </div>';
              else
                ...
                foreach ($review_list_data as $key => $value) 
                {
                    ...
                    if(($user_id ?? 0) != '')
                    {
                      ...
                      <div class="input-group collapse pt-2" id="collapsereview'.$value["id"].'">
                        ...
                        <button class="btn btn-primary btn-lg leave_review_comment no_radius" parent-id="'.$value['id'].'"><i class="fas -reply"></i> '.lang('Reply').'</button>              
                      </div>';
                    }
                    ...
                    if($value['review_reply']!='')
                    {
                      ...
                      $storeName = isset($social_analytics_codes['store_name']) ? $social_analytics_codes['store_name'] : lang('Admin');
                      $review_reply_show = '
          ...
          <ul class="list-group">        
            ...
            <li class="list-group-item d-flex justify-content-between align-items-center no_radius">
              ...
              <span class="badge badge-primary badge-pill">
                <?php echo isset($category_list[$product_data['category_id']]) ? $category_list[$product_data['category_id']] : lang('Uncategorised');?>
              </span>
            </li>
            <li class="list-group-item d-flex justify-content-between align-items-center no_radius">
              <?php echo lang('Sales'); ?>
            ...
            <div class="form-group m-0">
              <div class="input-group">
                <div class="input-group-append">
                  <button class="btn btn-dark add_to_cart" data-product-id="<?php echo $product_data['id'];?>" data-attributes="<?php echo $product_data['attribute_ids'];?>" data-action="remove" type="button" style="min-width: 120px;" data-toggle="tooltip" title="<?php echo lang('Remove 1 from Cart'); ?>"><i class="fas fa-minus-circle"></i> <?php echo lang('Remove'); ?> <?php echo lang('(-1)');?></button>
                </div>
                <input type="text" class="form-control text-center bg-white" data-toggle="tooltip" title="<?php echo lang('Currently added to cart');?>" id="item_count" readonly value="<?php echo $quantity_in_cart;?>">
                <div class="input-group-append">
                  <button style="min-width: 120px;" class="btn btn-primary add_to_cart no_radius" data-product-id="<?php echo $product_data['id'];?>"  data-attributes="<?php echo $product_data['attribute_ids'];?>" data-action="add" type="button" data-toggle="tooltip" title="<?php echo lang('Add 1 to Cart');?>"><i class="fas fa-cart-plus"></i> <?php echo lang('Add');?> <?php echo lang('(+1)');?></button>
                </div>
              </div>
            </div>  
          </div>
    ...
    <?php if(!empty($upsell_product_lists)) : ?>
      ...
      <div class="row section" id="upsell_product" style="display: none;">
        <div class="col-12 p-0">
          <div class="section-title mt-3 mb-3"><?php echo lang('You May Also Like') ?></div>
        </div>
            ...
            <div class="article-details pt-0 pb-0 pl-1 pr-1">
              ...
              <div>
                <?php if($downsell_product_id != 0): ?>
                  <a href="<?php echo $downsell_product_url; ?>" class="btn btn-outline-primary" data-toggle="tooltip" data-title="<?php echo lang('Cancel'); ?>"><i class="far fa-times-circle"></i></a>
                  <?php else : ?>
                  <a href="#" class="btn btn-outline-secondary disabled" data-toggle="tooltip" data-title="<?php echo lang('Cancel'); ?>"><i class="far fa-times-circle"></i></a>
                <?php endif; ?>
                <a href="<?php echo $upsell_product_url; ?>" class="btn btn-primary float-right" data-toggle="tooltip" data-title="<?php echo lang('Continue'); ?>"><i class="far fa-arrow-alt-circle-right"></i></a>  
              </div>
              &nbsp;   
            </div>
          </article>
      ...
      if(isset($ecommerce_review_comment_exist) && $ecommerce_review_comment_exist):      
        ...
        <div class="card mb-0 no_shadow">
          <div class="card-header p-0 pt-3 pb-3">
            <h4><i class="fas fa-paper-plane"></i>  <?php echo lang('Leave a comment');?></h4>
          </div>
          <div class="card-body p-0">
            <textarea id="new_comment" class="form-control comment_reply" placeholder="<?php echo lang('Write comment here');?>"></textarea>
            <button class="btn btn-primary btn-lg leave_comment mt-2" parent-id='' id="leave_comment"><i class="fas fa-comment"></i> <?php echo lang('Comment'); ?></button>
          </div>
        </div>
        <div class="card mt-2 mb-2 no_shadow" id="comment_section">
          <div class="card-header p-0 pt-3 pb-3">
            <h4><i class="fas fa-comment"></i> <?php echo lang('Comments');?></h4>
          </div>
          <div class="card-body p-0">
            ...
            <div class="card no_shadow m-0" id="nodata" style="display: none">
              <div class="card-body">
                <div class="empty-state p-0">
                  <h6 class="mt-0"><?php echo lang('We could not find any comment.') ?></h6>
                </div>
              </div>
            </div>            

            <button class="btn btn-outline-primary m-3 mb-4" style="display: none;" id="load_more" data-limit="10" data-start="0"><i class="fas fa-book-reader"></i> <?php echo lang('Load More'); ?></button>
  ...
  <?php if(isset($is_ecommerce_related_product_exist) && $is_ecommerce_related_product_exist) : ?>
    <?php if(!empty($related_product_lists)) : ?>
      <div class="row mt-2 section">
        <div class="col-12 p-0">
          <div class="section-title mt-3 mb-1"><?php echo lang('Related Items') ?></div>
        </div>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="ReviewModalLabel"><i class="fas fa-star"></i> <?php echo lang('Rate Item'); ?></h5>
      ...
      <div class="modal-body text-justify" id="ReviewModalBody">
              ...
              <div class="form-group">
                <label for=""><?php echo lang('Reason');?>*</label>
                <div class="input-group">
                    <input type="text" value="<?php echo isset($xreview[0]['reason'])?$xreview[0]['reason']:""; ?>" class="form-control" placeholder="<?php echo lang('Example : Quick Delivery'); ?>" id="reason" name="reason">
                </div>
              ...
              <div class="form-group">
                <label for=""><?php echo lang('Rating');?>*</label>
              ...
              <div class="form-group">
                <label for=""><?php echo lang('Review');?></label>
                <div class="input-group">
                    <textarea class="form-control" style="height: 200px !important" placeholder="<?php echo lang('Write a few words'); ?>" id="review" name="review"><?php echo isset($xreview[0]['review'])?$xreview[0]['review']:""; ?></textarea>
                </div>
                <div class="invalid-feedback"></div>
              </div>

              <div class="form-group">
                <label for=""><?php echo lang('Related Order');?> *</label>
                <div class="input-group">
                    <?php 
                    $related_order = array();
                    $default_related_order = isset($xreview[0]['cart_id'])?$xreview[0]['cart_id']:"";
                    foreach ($has_purchase_array as $key => $value) {
                      $related_order[$value["cart_id"]] = lang('Order')." #".$value["cart_id"];
                    }
