...
<section class="section section_custom">
	<div class="section-header">
		<h1><i class="fas fa-atom"></i> <?php echo lang('Sendinblue Integration'); ?></h1>
		
		<div class="section-header-button">
			<a class="btn btn-primary add_connector" id="add_feed" href="#" data-target="#sendinblue-integration-modal" data-toggle="modal">
				<i class="fas fa-plus-circle"></i> <?= lang('Add Account') ?>
			</a> 
		</div>

		<div class="section-header-breadcrumb">
		  <div class="breadcrumb-item active"><a href="<?php echo base_url('integration'); ?>"><?php echo lang('Integration'); ?></a></div>
						...
						<div class="table-responsive">
							<table id="sendinblue-datatable" class="table table-bordered" style="width:100%">
						        <thead>
						            <tr>
						                <th>#</th>
						                <th><?php echo lang('Tracking name'); ?></th>
    ...
    <div class="modal-dialog modal-md" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><i class="fa fa-plus-circle"></i> Sendinblue - <?= lang('Add Account') ?></h5>
            ...
            <div class="modal-body">
                <form class="" id="sendinblue-integration-form">
                    <div class="form-group">
                        <label><?= lang('Tracking Name') ?></label>
                        <div class="input-group">
                            ...
                            <input type="text" class="form-control" id="tracking-name" placeholder='<?php echo lang('Tracking Name'); ?>' name="tracking-name" autocomplete="off">
                        </div>
                    </div>
                    <div class="form-group">
		            ...
		            <div class="mt-5">           
		            	<button type="submit" class="btn btn-primary btn-shadow float-left" id="sendinblue-submit-button"><i class="fas fa-save"></i> <?= lang('Save') ?></button>
		            	<button type="button" class="btn btn-light btn-shadow float-right" data-dismiss="modal"><i class="fas fa-times"></i> <?= lang('Cancel') ?></button>
		            </div>
	            </form>
            </div>
        </div>
...
<div class="modal fade" role="dialog" id="sendinblue-details-modal" aria-hidden="true" data-backdrop="static" data-keyboard="false">
    <div class="modal-dialog" role="document" style="min-width:50%;">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><i class="fa fa-list"></i> Sendinblue - <?= lang('Account Details') ?></h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">               
                	<span aria-hidden="true">×</span>          
                </button>           
            </div>
            <div class="modal-body">
				<div class="card">
				    <div class="card-header bg-primary">
				        <h4 id="display-tracking-name" class="text-white"><?php echo lang('Test Account'); ?></h4>
				    </div>
...
<script>
	$(document).ready(function() {
		...
		$(document).on('click', '#sendinblue-delete-button', function(event) {
			...
			swal({
				title: 'Warning!',
				text: '<?= lang('Are you sure you want to delete this account?') ?>',
				icon: 'warning',
				buttons: true,
				dangerMode: true,
			}).then( yes => {
		...
		$(document).on('click', '#sendinblue-refresh-button', function(event) {
			...
			swal({
				title: 'Warning!',
				text: '<?= lang('Are you sure you want to refresh this account?') ?>',
				icon: 'warning',
				buttons: true,
				dangerMode: true,
			}).then( yes => {
