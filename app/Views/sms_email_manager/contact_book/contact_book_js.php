<script>
	$(document).ready(function($) 
	{
		...
		$(document).on('click', '#save_group', function(event) {
		    ...
		    if(group_name == '')
		    {
		        swal('<?php echo lang('Warning'); ?>', '<?php echo lang('Group Name is Required'); ?>', 'warning');
		        return;
		    }
		    ...
		    $.ajax({
		        ...
		        success:function(response)
		        {
		            ...
		            if(response == "1")
		            {
		                iziToast.success({title: '',message: '<?php echo lang('Contact Group has been created successfully.'); ?>',position: 'bottomRight'});
		            } else if(response == "2")
		            {
		            	iziToast.error({title: '',message: '<?php echo lang('Group Name Already Exists, please try with different one.'); ?>',position: 'bottomRight'});
		            } 
		            else 
		            {
		...
		$(document).on('click', '#update_group', function(event) {
		    ...
		    $.ajax({
		        ...
		        success:function(response)
		        {
	                ...
	                if(response == "1")
	                {
	                    iziToast.success({title: '',message: '<?php echo lang('Contact Group has been Updated successfully.'); ?>',position: 'bottomRight'});
	                } else if(response == "2")
	            	{
	            		iziToast.error({title: '',message: '<?php echo lang('Group Name Already Exists, please try with different one.'); ?>',position: 'bottomRight'});
	            	}  
		        ...
		        if (willDelete) 
		        {
		            ...
		            $.ajax({
		                ...
		                { 
		                    if(response == '1')
		                    {
		                        iziToast.success({title: '',message: '<?php echo lang('Contact Group has been deleted successfully.'); ?>',position: 'bottomRight',timeout: 3000});
		...
		$(document).on('click', '#save_contact', function(event) {
		    ...
		    {
		        swal('<?php echo lang('Warning'); ?>', '<?php echo lang('Email/Phone number is Required'); ?>', 'warning');
		        return;
		    }
		    ...
		    {
		        swal('<?php echo lang('Warning'); ?>', '<?php echo lang('Contact Group is Required'); ?>', 'warning');
		        return;
		    }
		...
		$(document).on('click', '#export_contact', function(event) {
		    ...
		    {
		        swal('<?php echo lang('Warning')?>', '<?php echo lang('You have to select Contacts to export.') ?>', 'warning');
		        return false;
		    }
		    else  
		    {
		...
		$(document).on('click', '#assign_sms_email_sequence', function(event) {
		  ...
		  if(ids=="") 
		  {
		    swal('<?php echo lang('Warning') ?>', '<?php echo lang('You have to select Contacts to assign sequence.') ?>', 'warning');
		    return;
		  } 
		...
		$(document).on('click','#assign_sequence_submit',function(e){
		  ...
		  if(count==0) 
		  {
		    swal('<?php echo lang('Error') ?>', '<?php echo lang('You have to select campaign to assign.') ?>', 'error');
		    return;
		  } 
		  ...
		  $.ajax({
		    ...
		    success:function(response)
		    {
		      ...
		      swal('<?php echo lang('Sequence Campaign Assign') ?>', '<?php echo lang('Sequence campaign have been assigned successfully'); ?>', 'success')
		      .then((value) => {
		...
		$(document).on('click', '#delete_all_contacts', function(event) {
			...
			if(contact_ids.length==0) {
			    ...
			    swal('<?php echo lang('Warning')?>', '<?php echo lang('You didn`t select any Contact to delete.') ?>', 'warning');
			    return false;
				    	...
				    	$.ajax({
				    	    ...
				    	    success:function(response){
				    	        ...
				    	        if(response == '1') {
				    	        	...
				    	        	iziToast.success({title: '',message: '<?php echo lang('Something went wrong, please try once again.'); ?>',position: 'bottomRight'});
		...
		$(document).on('click', '#assign_manual_sequence_submit', function(event) {
			...
			$.ajax({
				...
				success:function(response) {
					$(this).removeClass('btn-progress');
					iziToast.success({title: '',message: '<?php echo lang('Contact information has been saved successfully.'); ?>',position: 'bottomRight'});
				}
			})
			...
			$.ajax({
			  ...
			  success:function(response){
			     ...
			     if(response == "unsubscribed") {
			     	...
			     	iziToast.success({title: '',message: '<?php echo lang('Contact has been successfully unsubscribed.') ?>',position: 'bottomRight'});
			     	...
			     	iziToast.success({title: '',message: '<?php echo lang('Contact has been successfully subscribed.') ?>',position: 'bottomRight'});

			     } else {

			     	iziToast.error({title: '',message: '<?php echo lang('Something went wrong, try once again.') ?>',position: 'bottomRight'});
			     }
			  }
			});
		});
		...
		$(document).on('click', '#upload_imported_csv', function(event) {
		    ...
		    if(csvFile == "")
		    {
		        swal('<?php echo lang('Warning'); ?>', '<?php echo lang('Please Upload Contact CSV file.'); ?>', 'warning');
		        return;
		    }
		    ...
		    $.ajax({
		        ...
		        success:function(response){
		            ...
		            {   
	                	...
	                	$("#success_message").html('<b>'+total_inserted+'</b> <?php echo lang('Contacts Has been Inserted and'); ?> <b>'+total_updated+'</b> <?php echo lang('Contacts Has been Updated.'); ?>');

	                	if(response.rejected == "1") {
	                		var totalRejected = response.total_rejected;
	                		var seethecontacts = '<b>'+totalRejected+'</b> <?php echo lang('Contacts Has been rejected from insertion because of either email or phone number has already exists in the database.'); ?>'
