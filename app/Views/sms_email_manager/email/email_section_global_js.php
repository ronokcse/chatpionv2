	...
	$(document).ready(function($) {
		...
		var firstName = function (context) {
		  ...
		  var button = ui.button({
		    contents: '<i class="fas fa-user"/> '+'<?php echo lang('First Name') ?>',
		    container: 'body',
		...
		var lastName = function (context) {
		  ...
		  // create button
		  var button = ui.button({
		    contents: '<i class="fas fa-user"/> '+'<?php echo lang('Last Name') ?>',
		    container: 'body',
		...
		var unsubscriberlink = function (context) {
		  ...
		  // create button
		  var button = ui.button({
		    contents: '<i class="fas fa-bell-slash"/> '+'<?php echo lang('Unsubscribe Link') ?>',
		    container: 'body',
		...
		$('#message').summernote({
			...
			placeholder: '<?php echo lang('Write your message here...'); ?>'
		});
		...
		$(document).on('click', '#send_test_email', function(event) {
			...
			{
			    swal('<?php echo lang('Warning'); ?>', '<?php echo lang('Please provide valid email address'); ?>', 'warning');
			    return;
			}
		...
		$(document).on('click', '#save_smtp', function(event) {
			...
			{
			    swal('<?php echo lang('Warning'); ?>', '<?php echo lang('SMTP Email is Required'); ?>', 'warning');
			    return;
			}
			...
			{
			    swal('<?php echo lang('Warning'); ?>', '<?php echo lang('SMTP Host is Required'); ?>', 'warning');
			    return;
			}
			if(smtp_port == "" || smtp_port == null)
			{
			    swal('<?php echo lang('Warning'); ?>', '<?php echo lang('SMTP Port is Required'); ?>', 'warning');
			    return;
			}
			if(smtp_username == "" || smtp_username == null)
			{
			    swal('<?php echo lang('Warning'); ?>', '<?php echo lang('SMTP Username is Required'); ?>', 'warning');
			    return;
			}
			if(smtp_password == "" || smtp_password == null)
			{
			    swal('<?php echo lang('Warning'); ?>', '<?php echo lang('SMTP Password is Required'); ?>', 'warning');
			    return;
			}
			if(smtp_type == "")
			{
			    swal('<?php echo lang('Warning'); ?>', '<?php echo lang('SMTP Type is Required'); ?>', 'warning');
			    return;
			}
		        ...
		        if (willDelete) 
		        {
		            ...
		            $.ajax({
		                ...
		                success:function(response){ 
		                    ...
		                    if(response == '1')
		                    {
		                        iziToast.success({title: '',message: '<?php echo lang('API Information has been Deleted Successfully.'); ?>',position: 'bottomRight'});
		                        table1.draw();
		                    } else
		                    {
		...
		$(document).on('click', '#save_mailgun', function(event) {
			...
			{
			    swal('<?php echo lang('Warning'); ?>', '<?php echo lang('Email is Required'); ?>', 'warning');
			    return;
			}
			...
			{
			    swal('<?php echo lang('Warning'); ?>', '<?php echo lang('Domain is Required'); ?>', 'warning');
			    return;
			}
			if(mailgun_api_key == "" || mailgun_api_key == null)
			{
			    swal('<?php echo lang('Warning'); ?>', '<?php echo lang('API Key is Required'); ?>', 'warning');
			    return;
			}
		...
		$(document).on('click', '#save_mandrill', function(event) {
			...
			if(name == "" || name == null)
			{
			    swal('<?php echo lang('Warning'); ?>', '<?php echo lang('Name is Required'); ?>', 'warning');
			    return;
			}
		...
		$(document).on('click', '#save_sendgrid', function(event) {
			...
			{
			    swal('<?php echo lang('Warning'); ?>', '<?php echo lang('Username is Required'); ?>', 'warning');
			    return;
			}
			...
			{
			    swal('<?php echo lang('Warning'); ?>', '<?php echo lang('Password is Required'); ?>', 'warning');
			    return;
			}
		...
		$(document).on('click','#create_campaign',function(){
		    ...
		    if(campaign_name =='') {
		        swal('<?php echo lang('Warning'); ?>', "<?php echo lang('Please give a campaign name'); ?>", 'warning');
		        return;
		    }
		    ...
		    if(email_subject =='') {
		        swal('<?php echo lang('Warning'); ?>', "<?php echo lang('Email Subject is Required'); ?>", 'warning');
		        return;
		    }

		    /* Email Message field can't be empty */
		    if('' === message && '' === template_id) {
		        swal('<?php echo lang('Warning'); ?>', "<?php echo lang('Please write your message or select an email template.'); ?>", 'warning');
		        return;
		    }

		    /* Must select an email API to send emails */
		    if(from_email =='') {
		        swal('<?php echo lang('Warning'); ?>', "<?php echo lang('Please select a Email API'); ?>", 'warning');
		        return;
		    }

		    // contact group and manual number
		    if(page_name=="" && (contacts_id == "" || contacts_id == null || typeof(contacts_id) == 'undefined')) {
		        swal('<?php echo lang('Warning'); ?>', "<?php echo lang('Please Select Contacts from Page or Contact Groups.'); ?>", 'warning');
		        return;
		...
		$(document).on('click','#update_campaign',function(){
		    ...
		    /* Email Message field can't be empty */
		    if('' === message && '' === template_id) {
		        swal('<?php echo lang('Warning'); ?>', "<?php echo lang('Please write your message'); ?>", 'warning');
		        return;
		    }
		...
		$(document).on('click', '#updateMessage', function(event) {
		    ...
		    if(message == "") {
		        ...
		        swal('<?php echo lang('Warning'); ?>', "<?php echo lang('Please type a message. System can not send blank message.'); ?>", 'warning');
		        return;
		    ...
		    $.ajax({
		        ...
		        { 
		            ...
		            if(response == "1")
		            {
		                swal({ title:'<?php echo lang('Campaign Updated'); ?>', content:'<?php echo lang('Campaign have been updated successfully.'); ?>',icon:'success'}).then((value) => {window.location.href=report_link;});
		            }
		        }
		    });
		...
		$(document).on('click','.email_restart_button',function(e){
		    ...
		    swal({
		        ...
		        text: '<?php echo lang('Do you want to resume this campaign?') ?>',
		        icon: 'warning',
		        buttons: true,
		        dangerMode: true,
		    })
		...
		$(document).on('click','.pause_email_campaign_info',function(e){
		    ...
		    swal({
		        ...
		        text: '<?php echo lang('Do you want to pause this campaign?'); ?>',
		        icon: 'warning',
		        buttons: true,
		        dangerMode: true,
		    })
		...
		$(document).on('click','.play_email_campaign_info',function(e){
		    e.preventDefault();
		    var table_id = $(this).attr('table_id');
		    var somethingwentwrong = '<?php echo lang('Something went wrong, please try once again.'); ?>';

		    swal({
		        title: '<?php echo lang('Resume Campaign'); ?>',
		        text: '<?php echo lang('Do you want to start this campaign?'); ?>',
		        icon: 'warning',
		        buttons: true,
		        dangerMode: true,
		    })
        ...
        function prepare_select_options(data, selected = null) {
            ...
            var output = '<option value=""><?php echo lang('Select template'); ?></option>';
