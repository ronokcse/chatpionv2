...
{				
	...
	**********/
	...
	public function delete_comment($comment_id,$post_access_token){
		...
		$url="https://graph.facebook.com/v20.0/{$comment_id}?method=delete&access_token={$post_access_token}";
	...
	public function hide_comment($comment_id,$post_access_token){
		$url="https://graph.facebook.com/v20.0/{$comment_id}?method=post&access_token={$post_access_token}&is_hidden=true";
		$results= $this->run_curl_for_fb($url);
		return json_decode($results,TRUE);
	}
	...
	{	
	  //	echo	$url="https://graph.facebook.com/v20.0/{$post_id}/insights?access_token={$post_access_token}"; 
	...
	public function delete_persistent_menu($post_access_token='',$media_type='')
	{
		...
		if($media_type=="fb")
			$url = "https://graph.facebook.com/v20.0/me/messenger_profile?access_token={$post_access_token}";
		else
			$url = "https://graph.facebook.com/v20.0/me/messenger_profile?access_token={$post_access_token}&platform=instagram";
	...
	{
		$url="https://graph.facebook.com/v20.0/me/messaging_feature_review?access_token={$post_access_token}";
		$headers = array("Content-type: application/json");
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, $url);
		curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
	...
	{
		$url="https://graph.facebook.com/v20.0/me/broadcast_reach_estimations?access_token={$post_access_token}&method=post";
		$ch = curl_init();
		$headers = array("Content-type: application/json");
	...
	{
		$url="https://graph.facebook.com/v20.0/{$reach_estimation_id}?access_token={$post_access_token}";
		$ch = curl_init();
		$headers = array("Content-type: application/json");
	...
	public function domain_whitelist($access_token='',$domain='')
	{
		...
		$url = "https://graph.facebook.com/v20.0/me/messenger_profile?fields=whitelisted_domains&access_token={$access_token}";
		$ch = curl_init();
	 	$headers = array("Content-type: application/json");
	 	curl_setopt($ch, CURLOPT_URL, $url);
	 	curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
	...
	{
		$url = "https://graph.facebook.com/v20.0/me/insights/?metric=page_messages_total_messaging_connections,page_messages_new_conversations_unique,page_messages_blocked_conversations_unique,page_messages_reported_conversations_unique&access_token={$access_token}&since={$from_date}&until={$to_date}";
    	$ch = curl_init();
    	$headers = array("Content-type: application/json");
    	curl_setopt($ch, CURLOPT_URL, $url);
    	curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
	...
	{
		$url="https://graph.facebook.com/v20.0/me/custom_labels?access_token={$page_access_token}";
	...
	{
		$url="https://graph.facebook.com/v20.0/{$label_id}/label?access_token={$page_access_token}";
	...
	{
		$url="https://graph.facebook.com/v20.0/{$label_id}?access_token={$page_access_token}";
		return $this->call_api_post('',$url,true);
	}

	function retrieve_label($page_access_token='')
	{
		$url="https://graph.facebook.com/v20.0/me/custom_labels?fields=page_label_name&access_token={$page_access_token}&limit=200";
		return $this->call_api_post('',$url,false);
	}

	function retrieve_level_of_psid($psid,$page_access_token){

		$url="https://graph.facebook.com/v20.0/{$psid}/custom_labels?fields=page_label_name&access_token=$page_access_token&limit=200";
		return $this->call_api_post('',$url,false);
	...
	function fb_like_comment_share($url,$access_token)
	{
		...
		$url="https://graph.facebook.com/v20.0/?id={$url}&fields=engagement,og_object&access_token={$access_token}";
	...
	public function location_search($access_token,$keyword,$latitude,$longitude,$distance,$search_limit){
		...
		$url="https://graph.facebook.com/v20.0/search?q={$keyword}&type=place&access_token={$access_token}&fields=id,name,overall_star_rating,website,about,category_list,checkins,cover,description,engagement,hours,is_always_open,is_permanently_closed,payment_options,price_range,rating_count,restaurant_services,is_verified,location,link,phone&center={$center}&distance={$distance}&limit={$search_limit}";
	...
	public function add_ice_breakers($post_access_token='',$icebreakers_content_json='',$social_media_type='fb')
	{
		...
		if($social_media_type=='ig'){
			$url = "https://graph.facebook.com/v20.0/me/messenger_profile?platform=instagram&access_token={$post_access_token}";
	...
	{
		$url="https://graph.facebook.com/v20.0/{$comment_id}?method=post&access_token={$post_access_token}&hide=true";
		$results= $this->run_curl_for_fb($url);
		return json_decode($results,TRUE);
	}
