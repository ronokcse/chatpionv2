...
namespace App\Modules\Woocommerce_abandoned_cart\Controllers;
...
Modules:
{
   "266":{
      ...
      "limit_enabled":"1",
      "extra_text":"",
      "module_name":"E-commerce : WooCommerce Abandoned Cart Recovery"
   }
}
Project ID: 33
...
class Woocommerce_abandoned_cart extends Home
{
  ...
  public function initController(RequestInterface $request, ResponseInterface $response, LoggerInterface $logger)
  {
    ...
    public function webhook_callback($verification_code='')
    {
      ...
      $response_raw=$this->request->getPost('response_raw');
    ...
    public function messenger_checkbox_plugin() // echo js getting domain code
    {
        ...
        if($code=="") 
        {
            echo "console.error('".config('App')->product_name." Error : Facebook messenger checkbox plugin is failed to load, no domain code found.');";
            exit();
        }
        ...
        {
            echo "console.error('".config('App')->product_name." Error : Facebook messenger checkbox plugin is failed to load, invalid domain code.');";
            exit();
        }
        ...
        {
            echo "console.error('".config('App')->product_name." Error : Facebook messenger checkbox plugin is failed to load, the requesting user is no longer valid.');";
            exit();
        }
    ...
    {
        if(session()->get('user_type') != 'Admin' && (isset($this->module_access) && is_array($this->module_access) && !in_array(266,$this->module_access)))
        {
            redirect()->to('home/login_page')->send();
            exit();
        }
        ...
        $data['page_title'] = lang("WooCommerce Abandoned Cart Recovery");
        ...
        session()->set("woocommerce_selected_plugin",$plugin_id);
        ...
        if($this->request->getPost('from_date')=="") $from_date=$from_date." 00:00:00";
        if($this->request->getPost('to_date')=="") $to_date=$to_date." 23:59:59";

        session()->set("woocommerce_from_date",$from_date);
        session()->set("woocommerce_to_date",$to_date);
        ...
        if(isset($info[$i]))
        {
          $info[$i]["domain_name"] = "<a data-toggle='tooltip' data-original-title='".lang("Visit store")."' target='_BLANK' href='".addHttp($info[$i]["domain_name"])."'>".$info[$i]["domain_name"]."</a>";
          ...
          $info[$i]['download'] = '<a href="'.base_url("woocommerce_abandoned_cart/download_woocommerce_plugin/".$info[$i]["id"]).'" class="badge badge-status"><i class="fas fa-cloud-download-alt"></i> '.lang('Download').'</a>';
          ...
          $info[$i]['actions'] = '<div style="min-width:100px;"><a class="btn btn-circle btn-outline-warning" href="'.base_url()."woocommerce_abandoned_cart/recovery_plugin_edit/".$info[$i]['id'].'" data-toggle="tooltip" title="'.lang('Edit').'" campaign_id='.$info[$i]['id'].'><i class="fas fa-edit"></i></a> &nbsp;';
          $info[$i]['actions'] .= '<a class="btn btn-circle btn-outline-primary download" data-toggle="tooltip" href="#" title="'.lang('download').'" campaign_id='.$info[$i]['id'].'><i class="fas fa-cloud-download-alt"></i></a> &nbsp;';
          $info[$i]['actions'] .= '<a class="btn btn-circle btn-outline-danger delete_campaign" data-toggle="tooltip" href="#" title="'.lang('delete').'" campaign_id='.$info[$i]['id'].'><i class="fas fa-trash-alt"></i></a>';
        }       
    ...
    {
      $this->ajax_check();
      $id = $this->request->getPost('webhook_id');
      ...
      {
        echo '<div class="alert alert-danger text-center">'.lang("Something went wrong, activity data not found.").'</div>';
        exit();
      }
              ...
              <div class="invoice-print">
                ...
                <div class="row mt-4">
                  <div class="col-md-12">
                    ...
                    <div class="row mt-4">
                      <div class="col-lg-8">
                        <div class="section-title">'.lang("Coupon").'</div>
                        <p class="section-lead">'.$coupon_info.'</p>
                        <div class="section-title">'.lang("Payment Method").'</div>
      ...
      {
        $messenger_confirmation_badge = '<span class="badge badge-light badge-pill">'.lang("Unknown").'</span>';
        if(isset($confirmation_response['messenger']))
        {
          if(isset($confirmation_response['messenger']['status']) && $confirmation_response['messenger']['status']=='1') $messenger_confirmation_badge = '<span data-toggle="tooltip" title="'.htmlspecialchars($confirmation_response['messenger']['response']).'" class="badge badge-success badge-pill">'.lang("Sent").'</span>';
          else if(isset($confirmation_response['messenger']['status']) && $confirmation_response['messenger']['status']=='0') $messenger_confirmation_badge = '<span data-toggle="tooltip" title="'.htmlspecialchars($confirmation_response['messenger']['response']).'" class="badge badge-danger badge-pill">'.lang("Error").'</span>';
          else $messenger_confirmation_badge = '<span class="badge badge-dark badge-pill">'.lang("Not Set").'</span>';
        }
        $messenger_li = '<li class="list-group-item d-flex justify-content-between align-items-center">
                          '.lang("Messenger Confirmation").'
                          '.$messenger_confirmation_badge.'
                        </li>';

        $sms_li = $email_li = "";
        if(session()->get('user_type') == 'Admin' || (isset($this->module_access) && is_array($this->module_access) && in_array(264,$this->module_access))) 
        {
          $sms_confirmation_badge = '<span class="badge badge-light badge-pill">'.lang("Unknown").'</span>';
          if(isset($confirmation_response['sms']))
          {
            if(isset($confirmation_response['sms']['status']) && $confirmation_response['sms']['status']=='1') $sms_confirmation_badge = '<span data-toggle="tooltip" title="'.htmlspecialchars($confirmation_response['sms']['response']).'" class="badge badge-success badge-pill">'.lang("Sent").'</span>';
            else if(isset($confirmation_response['sms']['status']) && $confirmation_response['sms']['status']=='0') $sms_confirmation_badge = '<span data-toggle="tooltip" title="'.htmlspecialchars($confirmation_response['sms']['response']).'" class="badge badge-danger badge-pill">'.lang("Error").'</span>';
            else $sms_confirmation_badge = '<span class="badge badge-dark badge-pill">'.lang("Not Set").'</span>';
          }
          $sms_li = '<li class="list-group-item d-flex justify-content-between align-items-center">
                      '.lang("SMS Confirmation").'
                      '.$sms_confirmation_badge.'
                    </li>';
        }

        if(session()->get('user_type') == 'Admin' || (isset($this->module_access) && is_array($this->module_access) && in_array(263,$this->module_access))) 
        {
          $email_confirmation_badge = '<span class="badge badge-light badge-pill">'.lang("Unknown").'</span>';
          if(isset($confirmation_response['email']))
          {
            if(isset($confirmation_response['email']['status']) && $confirmation_response['email']['status']=='1') $email_confirmation_badge = '<span data-toggle="tooltip" title="'.htmlspecialchars($confirmation_response['email']['response']).'" class="badge badge-success badge-pill">'.lang("Sent").'</span>';
            else if(isset($confirmation_response['email']['status']) && $confirmation_response['email']['status']=='0') $email_confirmation_badge ='<span data-toggle="tooltip" title="'.htmlspecialchars($confirmation_response['email']['response']).'" class="badge badge-danger badge-pill">'. lang("Error").'</span>';
            else $email_confirmation_badge = '<span class="badge badge-dark badge-pill">'.lang("Not Set").'</span>';
          }
              ...
              <div class="invoice" style="border:1px solid #dee2e6;">
                <div class="invoice-print">
                  <div class="row">
                    <div class="col-12">
                      <div class="invoice-title">
                        <h4>'.lang("Checkout Confirmation").'</h4>
    ...
    {
      $this->ajax_check();
        if(session()->get('user_type') != 'Admin' && (isset($this->module_access) && is_array($this->module_access) && !in_array(266,$this->module_access))) exit();
      $auto_id = $this->request->getPost("campaign_id");
      $join = array('messenger_bot_engagement_checkbox'=>"messenger_bot_engagement_checkbox.id=woocommerce_drip_campaign.engagement_table_id,left");
      ...
      if(!isset($plugin_data[0]))
      {
        echo json_encode(array('status'=>'0','message'=>lang("Something went wrong. We could not find plugin data.")));
        exit();
      }
      ...
      $message='';
      $message .='<div class="card text-center" style="margin:0 auto; border:1px solid #ddd;"> <div class="card-body"><a href="'.$download_url.'"><i class="fas fa-cloud-download-alt" style="font-size: 50px;"></i></a></div> <div class="card-footer bg-whitesmoke"><a href="'.$download_url.'">'.lang("Download Plugin").'</a></div></div>';
      $message .= '
      <br>
      <div class="list-group">
        <span href="#" class="list-group-item list-group-item-action flex-column align-items-start">
          <div class="d-flex w-100 justify-content-between">
            <h6 class="mb-1">'.lang("Verification Code").'</h6>
          </div>
        ...
        <span href="#" class="list-group-item list-group-item-action flex-column align-items-start">
          <div class="d-flex w-100 justify-content-between">
            <h6 class="mb-1"> '.lang("Webhook Call URL").'</h6>
          </div>
        ...
        <span href="#" class="list-group-item list-group-item-action flex-column align-items-start">
          <div class="d-flex w-100 justify-content-between">
            <h6 class="mb-1"> '.lang("Checkbox Script URL").'</h6>
          </div>
    ...
    public function download_woocommerce_plugin($auto_id='0')
    {
        ...
        {
           redirect()->to('woocommerce_abandoned_cart/zip_error')->send();
           exit();
        }
        ...
        if(!isset($plugin_data[0]))
        {
           redirect()->to('woocommerce_abandoned_cart/zip_error/1')->send();
           exit();        
        }
        ...
        $rootPath = FCPATH.'app/modules/woocommerce_abandoned_cart/plugin';
    ...
    public function recovery_plugin_add()
    {
        ...
        $data['page_title'] = lang("Add WooCommerce Abandoned Cart Plugin");
    ...
    public function recovery_plugin_add_action()
    {
        ...
        if(!$anything_found)
        {
          echo json_encode(array('status'=>'0','message'=>lang('You have not set any reminder.')));
          exit();
        }  
        ...
        {
            echo json_encode(array('status'=>'1','message'=>lang('Almost done. The next step is to download plugin and install in your WordPress website.'),'id'=>$insert_id));
            exit();
        } 
    }
    ...
    public function recovery_plugin_edit($id=0)
    {
        ...
        $data['page_title'] = lang("Edit WooCommerce Abandoned Cart Plugin");
    ...
    public function recovery_plugin_edit_action()
    {
        ...
        {
            echo json_encode(array('status'=>'1','message'=>lang('Plugin has been updated successfully.'),'id'=>$engagement_table_id));
            exit();
        } 
    }
    ...
    { 
        ...
        $data_days = 30; 
        $from_date = session()->get("woocommerce_from_date");
        $to_date = session()->get("woocommerce_to_date");
        if($to_date=='') $to_date = date("Y-m-d");
        if($from_date=='') $from_date = date("Y-m-d",strtotime("$to_date - ".$data_days." days"));
        ...
        $drip_data = $this->basic->get_data("woocommerce_drip_campaign",array("where"=>array("engagement_table_id"=>session()->get("woocommerce_selected_plugin"))),"id");
        $woocommerce_drip_campaign_id = isset($drip_data[0]['id']) ? $drip_data[0]['id'] : 0;
        ...
        foreach($info as $key => $value) 
        {
            ...
            else $sent_time = '<span class="text-muted"><i class="fas fa-exclamation-circle"></i> '.lang("Not Sent")."<span>";
            ...
            $info[$key]['response'] =  "<a class='btn btn-sm btn-outline-primary woo_error_log' href='' data-id='".$info[$key]['id']."'><i class='fas fa-plug'></i> ".lang('Response')."</a>";
        }
    ...
    {
        if($param==1)
        echo "<h2 class='text-align:center;color:red;border:1px solid red;padding:20px;margin-top:30px;'>".lang("WooCommerce Abandoned Cart Recover plugin is not found.");
        else echo "<h2 class='text-align:center;color:red;border:1px solid red;padding:20px;margin-top:30px;'>".lang("WooCommerce Abandoned Cart Recover plugin can not be generated beacuse PHP ZipArchive class is not installed.");
    }
    ...
    private function get_sms_api()
    {
        ...
        $sms_api_config_option=array(''=>lang("Select Sender"));
    ...
    private function get_email_api()
    {
      ...
      $smtp_option=array(''=>lang("Select Sender"));
    ...
    private function get_reminder_hour()
    {
      return array(
          "" => "--".lang("Do not send")."--",
          "1"=>lang("After 1 hour"),
          "2"=>lang("After 2 hours"),
          "3"=>lang("After 3 hours"),
          "4"=>lang("After 4 hours"),
          "5"=>lang("After 5 hours"),
          "6"=>lang("After 6 hours"),
          "7"=>lang("After 7 hours"),
          "8"=>lang("After 8 hours"),
          "9"=>lang("After 9 hours"),
          "10"=>lang("After 10 hours"),
          "11"=>lang("After 11 hours"),
          "12"=>lang("After 12 hours"),
          "13"=>lang("After 13 hours"),
          "14"=>lang("After 14 hours"),
          "15"=>lang("After 15 hours"),
          "16"=>lang("After 16 hours"),
          "17"=>lang("After 17 hours"),
          "18"=>lang("After 18 hours"),
          "19"=>lang("After 19 hours"),
          "20"=>lang("After 20 hours"),
          "21"=>lang("After 21 hours"),
          "22"=>lang("After 22 hours"),
          "23"=>lang("After 23 hours"),
        );
    }
