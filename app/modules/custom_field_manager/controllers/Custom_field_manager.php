...
namespace App\Modules\Custom_field_manager\Controllers;
...
class Custom_field_manager extends Home
{
  ...
  { 
    ...
    $data['page_title'] = lang("Custom field list");
    if($media_type =="ig")
    $data['page_title'] = lang("Instagram Custom field list");
  ...
  { 
    ...
    for($i=0;$i<count($info);$i++) 
    {
      ...
      $info[$i]['actions'] = "<a href='#' class='btn btn-outline-danger btn-circle delete_custom_field' media_type='".$media_type."' table_id='".$info[$i]['id']."' title='".lang("Delete Custom Field")."'><i class='fas fa-trash-alt'></i></a>";
    }
  ...
  public function ajax_custom_field_insert()
  {
    ...
    if(empty($custom_field_info))
    {
      ...
      if($this->basic->insert_data("user_input_custom_fields",$insert_data))
      {
        ...
        $str = "<option value=''>".lang('Please select')."</option>";
    ...
    else
    {
      ...
      $return['status'] = "1";
      $return['message'] = lang("Custom field been updated successfully.");
    }
  ...
  public function ajax_delete_custom_field()
  {
    ...
    {
        echo json_encode(array("status" => "0", "message" =>lang("Database error occured during delete.")));
        exit();
    }
    else
    {
      $return['status'] = 'successfull';
      $return['message'] = lang("Custom field has been deleted successfully.");
      echo json_encode($return); 
    }
  ...
  public function campaign_list($page_id=0,$iframe='0',$media_type="fb")
  {
    ...
    $data['media_type'] = $media_type;
    $data['page_title'] = lang("Facebook Flow Campaign list");
    if($media_type=="ig")
      $data['page_title'] = lang("Instagram Flow Campaign list");
  ...
  { 
    ...
    for($i=0;$i<count($info);$i++) 
    {
      ...
      // $edit_url = "<a href='".base_url("custom_field_manager/edit_question_content/").$info[$i]['id']."/".$media_type."' class='btn btn-outline-warning btn-circle' data-toggle='tooltip' title='".lang("Edit Campaign")."'>";

      $edit_url = $delete_url = '';
      $edit_url = "<a href='".base_url("custom_field_manager/edit_question_content/").$info[$i]['id']."/1/".$media_type."' class='btn btn-outline-warning btn-circle' data-toggle='tooltip' title='".lang("Edit Campaign")."'><i class='fas fa-edit'></i></a>&nbsp;";
      if($info[$i]['visual_flow_type'] == 'flow')
      {
        ...
        if(!empty($flow_campaign_exist)) {
          $edit_url = "<a target='_BLANK' href='".base_url("visual_flow_builder/edit_builder_data/").$info[$i]['visual_flow_campaign_id']."/3/".$media_type."' class='btn btn-outline-warning btn-circle' data-toggle='tooltip' title='".lang("Edit Campaign")."'>  <i class='fas fa-edit'></i></a>&nbsp;";
        }
      }

      if($info[$i]['visual_flow_type'] == 'general') {
        $delete_url = "<a href='#' class='btn btn-outline-danger btn-circle delete_campaign' media_type='".$media_type."' table_id='".$info[$i]['id']."' data-toggle='tooltip' title='".lang("Delete Campaign")."'><i class='fas fa-trash-alt'></i></a>";
      }

      $info[$i]['actions'] = "<a href='#' class='btn btn-outline-info btn-circle view_report' media_type='".$media_type."' table_id='".$info[$i]['id']."' data-toggle='tooltip' title='".lang("Report")."'><i class='fas fa-eye'></i></a>&nbsp;<a data-toggle='tooltip' title='".lang('Export flow data')."' href='#' class='btn btn-circle btn-outline-success export_data' media_type='".$media_type."' table_id='". $info[$i]['id'] . "'><i class='fas fa-file-export'></i></a>&nbsp;".$edit_url.$delete_url." <script>$('[data-toggle=\"tooltip\"]').tooltip();</script>";
  ...
  public function ajax_delete_flow_campaign()
  {
    ...
    if ($this->db->trans_status() === FALSE)
    {
        echo json_encode(array("status" => "0", "message" =>lang("Deleting campaign was unsuccessful. Database error occured during campaign delete.")));
        exit();
    }
    else
    {
      $return['status'] = 'successfull';
      $return['message'] = lang("Flow campaign and all of it's corresponding data has been deleted successfully.");
      echo json_encode($return); 
    }
  ...
  public function get_submitted_subscribers()
  {
    ...
    foreach ($info as $key => $value) 
    {
      ...
      $info[$key]['actions']='<a href="#" class="btn btn-circle btn-outline-info get_subscriber_formdata" data-id="'.$value['id'].'" subscribe_id="'.$value['subscribe_id'].'" page_table_id="'.$value['page_table_id'].'" data-form-id="'.$table_id.'" title="'.lang('View Form Data').'" ><i class="fas fa-eye"></i></a>&nbsp;<a target="_BLANK" href="'.base_url('subscriber_manager/bot_subscribers/').$value['subscribe_id'].'" class="btn btn-circle btn-outline-warning" title="'.lang('Go to subscriber list').'" ><i class="far fa-hand-point-right"></i></a>';
  ...
  public function get_subscriber_formdata()
  {
    ...
    $content = '<div class="card w-100 no_shadow">
      ...
      foreach($info as $value)
      {
        ...
        if($substr == 'https://') 
        {
          $answer = "<a target='_BLANK' href='".$value["user_answer"]."'>".lang('Visit Link')."</a>";
        }
  ...
  public function input_flow_builder($page_id=0,$iframe='0',$media_type="fb")
  {
    ...
    $data['page_title'] = lang("Create Facebook User Input Flow");
    if($media_type =="ig") {
      $data['page_title'] = lang("Create Instagram User Input Flow");
    }
    $this->_viewcontroller($data); 
  }
  ...
  public function ajax_add_question_content()
  {
    ...
    {
      $custom_fileds_content .= "<option value=''>".lang('Please select')."</option>";
      foreach ($custom_field_list as $value) {
        $custom_fileds_content .= "<option value='".$value["id"]."'>".$value["name"]."</option>";
      }
    }
    else
      $custom_fileds_content .= '<option value="">'.lang('No custom field found').'</option>';
    ...
    {
      $user_labels .= "<option value=''>".lang('Please select')."</option>";
      foreach ($user_label_info as $value) {
        $user_labels .= "<option value='".$value["id"]."'>".$value["group_name"]."</option>";
      }
    }
    else
      $user_labels .= '<option value="">'.lang('No label found').'</option>';

    $messenger_sequence = '';
    if($this->addon_exist("messenger_bot_enhancers"))
    {
      if(session()->get('user_type') == 'Admin' || count(array_intersect($this->module_access, ['219']))!=0)
      {
        ...
        {
          $messenger_sequence .= "<option value=''>".lang('Please select')."</option>";
          foreach ($messenger_sequence_info as $value) {
            $messenger_sequence .= "<option value='".$value["id"]."'>".$value["campaign_name"]."</option>";
          }
        }
        else
          $messenger_sequence .= '<option value="">'.lang('No sequence campaign found').'</option>';
      }
    }
    ...
    if($this->addon_exist("sms_email_sequence"))
    {
      if(session()->get('user_type') == 'Admin' || count(array_intersect($this->module_access, ['270','271']))!=0)
      {
        ...
        {
          $email_phone_sequence .= "<option value=''>".lang('Please select')."</option>";
          ...
          $email_phone_sequence .= '<option value="">'.lang('No sequence campaign found').'</option>';
      }
    }
      ...
      $response .= '<div class="input_section" style="width:100%" id="input_section_'.$random_variable.'">
                      <div class="form-group">
                        <div class="input-group mb-2">
                          <input type="text" class="form-control type_questions" name="question['.$random_variable.']" placeholder="'.lang('Put Your Question Here').'">
                        ...
                        <div class="float-right free_input_label">'.lang('Free keyboard input').'</div>
                      </div>
                      <br>
                      <div class="waiting_reply_content">
                        <span>..... '.lang('Waiting for a reply from the user').' ....</span>
                      </div>
                    </div>';
    }
    else
                      ...
                      <div class="form-group mb-2">
                        ...
                        <div class="form-inline multiple_input_more_parent">
                          <div class="multiple_input_item" id="multiple_choice_buttons_'.$random_variable.'">
                            <input type="text" class="form-control mb-2 multiple_input_more" name="multiple_choice['.$random_variable.'][]" id="multiple_choice['.$random_variable.'][]" placeholder="'.lang("Option 1").'">
                            <input type="text" class="form-control mb-2 multiple_input_more" name="multiple_choice['.$random_variable.'][]" id="multiple_choice['.$random_variable.'][]" placeholder="'.lang("Option 2").'">
                            <input type="text" class="form-control mb-2 multiple_input_more" name="multiple_choice['.$random_variable.'][]" id="multiple_choice['.$random_variable.'][]" placeholder="'.lang("Option 3").'">
                            <input type="text" class="form-control mb-2 multiple_input_more" name="multiple_choice['.$random_variable.'][]" id="multiple_choice['.$random_variable.'][]" placeholder="'.lang("Option 4").'">
                          </div>
                        </div>
                      </div>
                      <div class="form-group mr-2">
                          <button type="" class="btn btn-sm btn-outline-primary float-right add_more_button" div_id="multiple_choice_buttons_'.$random_variable.'"><i class="fas fa-plus-circle"></i> '.lang('Add more').'</button>
                      </div><br>
                      <div class="waiting_reply_content">
                        <span>..... '.lang('Waiting for a reply from the user').' ....</span>
                      </div>
                ...
                <div class="card-body p-3">
                  ...
                  <div id="email_quickreply_checkbox_'.$random_variable.'" class="'.$display_class.' mb-1 '.$hide_quick_reply_checkbox.'">
                    <div class="custom-control custom-checkbox">
                        ...
                        <label class="custom-control-label" for="quickreply_checkbox['.$random_variable.'][]">'.lang("Attach Email Quick-reply").'</label>
                    </div>
                  </div>
                  ...
                  <div id="phone_quickreply_checkbox_'.$random_variable.'" class="'.$phone_checkbox_display_class.' mb-1 '.$hide_quick_reply_checkbox.'">
                    <div class="custom-control custom-checkbox">
                        ...
                        <label class="custom-control-label" for="phone_quickreply_checkbox['.$random_variable.'][]">'.lang("Attach Phone Quick-reply").'</label>
                    </div>
                  </div>

                  <div class="form-group mb-1" div_id="selected_custom_field_'.$random_variable.'">
                    <label>'.lang('Save to Custom Field').'</label>
                  ...
                  <div class="form-group mb-1" id="selected_system_field_'.$random_variable.'">
                    <label>'.lang('Save to System Field').'</label>
                  ...
                  <div class="form-group mb-1" id="assign_to_labels_'.$random_variable.'">
                    <label>'.lang('Assign to labels').'</label>
    ...
    if($this->addon_exist("messenger_bot_enhancers"))
    {
      if(session()->get('user_type') == 'Admin' || count(array_intersect($this->module_access, ['219']))!=0)
      {
        $response .= '<div class="form-group mb-1" id="assign_to_messenger_sequence_'.$random_variable.'">
          <label>'.lang('Assign to a Messenger Sequence').'</label>
    ...
    if($this->addon_exist("sms_email_sequence"))
    {
      if(session()->get('user_type') == 'Admin' || count(array_intersect($this->module_access, ['270','271']))!=0)
      {
        $response .= '<div class="form-group mb-1" id="assign_to_email_phone_sequence_'.$random_variable.'">
          <label>'.lang('Assign to a Email/Phone Sequence').'</label>
    ...
    $response .= '<div class="form-group mb-1" id="skip_button_field_'.$random_variable.'">
                    <label>'.lang('Skip button text').'</label>
                    <input type="text" class="form-control" name="skip_button_text['.$random_variable.']" placeholder="'.lang('Put your skip button text here').'">
                  </div>

                </div>

                <div class="card-footer text-center pt-0">
                  <a href="#" class="btn btn-icon btn-sm icon-left btn-danger delete_single_block" single_block_div_id="block_'.$random_variable.'"><i class="fas fa-times"></i> '.lang('Remove This Question').'</a>
                </div>
              </div>
            </div>
          </div>
  ...
  public function question_submit()
  {
    ...
    if($status=="2") 
    {
        $error_msg = lang("sorry, your bulk limit is exceeded for this module.")."<a href='".site_url('payment/usage_history')."'>".lang("click here to see usage log")."</a>";
    ...
    {
        $error_msg = lang("sorry, your monthly limit is exceeded for this module.")."<a href='".site_url('payment/usage_history')."'>".lang("click here to see usage log")."</a>";
        $return_val=array("status"=>"0","message"=>$error_msg);
        echo json_encode($return_val);
        exit();
    }
    ...
    if ($this->db->trans_status() === FALSE)
    {
      echo json_encode(array("status" => "0", "message" =>lang("Creating campaign was unsuccessful. Database error occured during campaign create.")));
        exit();
    }
    else
    {
  ...
  public function get_postback_dropdown()
  {
    ...
    $push_postback = "<option value=''>".lang("Select a postback")."</option>";
  ...
  public function edit_question_content($id=0,$iframe='0',$media_type="fb")
  {
    ...
    $data['iframe'] = $iframe;
    $data['page_title'] = lang("Edit User Input Flow");
  ...
  public function edit_question_submit()
  {
    ...
    if ($this->db->trans_status() === FALSE)
    {
        echo json_encode(array("status" => "0", "message" =>lang("Updating campaign was unsuccessful. Database error occured during update.")));
        exit();
    }
    else
    {
  ...
  public function ajax_get_variables()
  {
    ...
    $media_type = 'fb';
    $media_type = $this->input->post("media_type",true);
    $content = '<div class="section-title">'.lang('Variables you currently have').'</div>';
    $custom_fields_array = [];
    ...
    foreach ($custom_fields_array as $key => $value) :
      $content .= '<p>'.lang('Reply Type').': <b><i>'.$key.'</i> </b></p>
      <p>
        '.lang('Variables').': 
        <b>'.implode(' , ', $value).'</b>
      </p>';
  ...
  public function export_flow_data() 
  {
    ...
    if(!check_module_action_access($module_id=292,$actions=4,'check')){
      echo json_encode(['error' => lang("You do not have access to perform this action.")]);
      exit; 
    }
    ...
    if ('post' == strtolower($method)) {
      ...
      if (empty($form)) {
        $message = lang('No flow data to be exported.');
        echo json_encode(['info' => $message]);
        exit;
      }

      // Sets form ID into session
      session()->set('inputflow_export_form_data_form_id', $table_id);
      ...
      $table_id = session()->get('inputflow_export_form_data_form_id');
      ...
      // Unsets form ID from session
            session()->remove('inputflow_export_form_data_form_id');
            exit;
    }
  }
