			        	...
			        	<div class="row">
			        		<div class="col-md-9 col-12">
				            	<div class="input-group mb-3" id="searchbox"><!-- search by page name -->
	          	  	          	    ...
	          	  	          	    <input type="text" class="form-control" id="campaign_name" autofocus placeholder="<?php echo lang('campaign name'); ?>" aria-label="" aria-describedby="basic-addon2">
...
$(document).ready(function($) {
    ...
    $(document).on('click','.delete',function(e){
      ...
      swal({
        ...
        text: "<?php echo lang('Do you really want to delete this post from the database?'); ?>",
        icon: 'warning',
        buttons: true,
        dangerMode: true,
      })
    ...
    $(document).on('click','.delete_p',function(e){
      ...
      swal({
        title: '<?php echo lang("Are you sure?"); ?>',
        text: "<?php echo lang('This is main campaign, if you want to delete it, rest of the sub campaign will be deleted. Do you really want to delete this post from the database?'); ?>",
        icon: 'warning',
        buttons: true,
        dangerMode: true,
      })
	...
	$(document).on('click', '.not_see_report', function(event) {
	  event.preventDefault();
	  swal("","<?php echo lang('Sorry, Only parent campaign has shown report.'); ?>","error");
	});

	$(document).on('click', '.not_published', function(event) {
	  event.preventDefault();
	  swal("","<?php echo lang('Sorry, this post is not published yet.'); ?>",'error');
	});

	$(document).on('click', '.not_editable', function(event) {
	  event.preventDefault();
	  swal("","<?php echo lang('Sorry, Only Pending Campaigns Are Editable.'); ?>",'error');
	});
	...
	$(document).on('click', '.not_embed_code', function(event) {
	  event.preventDefault();
	  swal("","<?php echo lang('Sorry, Embed code is only available for published video posts.'); ?>",'error');
	});
    ...
    <div class="modal-dialog modal-mega">
        <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title"><i class="fas fa-window-restore"></i> <?php echo lang("Report of Video Slide Show/Carousel Post");?></h5>
                      ...
                      <table class="table table-bordered" id="mytable1">
                          <thead>
                            <tr>
                              ...
                              <th><?php echo lang("posting page or group"); ?></th>
                              <th><?php echo lang("Post Type"); ?></th>
                              <th><?php echo lang("Post ID"); ?></th>
                              <th><?php echo lang("Posting Status"); ?></th>
                              <th><?php echo lang("Schedule Time"); ?></th>
                              <th><?php echo lang("Error"); ?></th>
                            </tr>
                          </thead>
                      </table>
...
<div class="modal fade" id="view_report" data-backdrop="static" data-keyboard="false">
    <div class="modal-dialog  modal-lg" style="min-width: 70%;max-width: 100%;">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                <h4 class="modal-title text-center"><i class="fa fa-list-alt"></i> <?php echo lang("report of Video Slide Show/Carousel Post") ?></h4>
            </div>
